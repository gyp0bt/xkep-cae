...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7+10 テスト** — 2026-04-25 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-372（候補 (g1) active 履歴 EMA 平滑化 α 掃引 — 7 本部分達成 / 19 本却下）**。status-371 引継ぎ最優先 TODO に対応、α ∈ {0.0, 0.1, 0.3, 0.5} を **7 本 / 19 本撚線 90° 曲げ**で実機掃引。**7 本**: α=0.30/0.50 で frac=1.0 維持 + cb 57→14/22（**-61〜-75%**）、α=0.50 で elapsed -11%（298→265s）、α=0.10 のみ早期 stall（frac=0.3350、弱平滑化逆効果）。**19 本**: gate「frac ≥ 0.6」**全ケース未達**で候補 (g1) **却下方向**、α=0.50 で frac=0.5133（baseline 0.3739 比 +37.3%）の部分改善も elapsed +131% でコスト過大、α=0.10/0.30 は -41%/-47% 退化。**default 変更なし**: `StrandBendingOscillationConfig.active_ema_alpha` default=0.0 維持、`active_ema_alpha=0.5` を 7 本系 cutback 削減 opt-in escape hatch として `docs/roadmap.md` opt-in 表（5 項目目）に追加（status-368 `chattering_freeze_nr_max=30` と同パターン）。`26_active_ema_alpha_sweep.py` docstring に 8 ケース実測結果を埋込。次候補 **(g3) pair-wise relaxation**（status-284 接触凍結を pair granularity 拡張）→ (g2) AL 再導入。実装本体（`xkep_cae/`、`tests/`、`contracts/`）は**無変更**。gate: 契約違反 **0 件**（全 24 検査 OK）/ 条例違反 **0 件** / `pytest xkep_cae/contact/` **456 passed 5 skipped**（status-371 維持）/ mathematics 109 passed / `test_helical_3d_hermite` rel_err=2.18e-07 維持 / ruff check + format pass。Phase A〜E / status-346〜372 の **23/N 完了**
...
- **status-373（次セッション・候補 (g3) pair-wise relaxation 着手）**: (1) **最優先**: status-284 接触凍結モードを pair granularity 拡張する設計仕様を `xkep_cae/mathematics/docs/phase_c3prime_19strand_plan.md` §3.2 に追記（チャタリング pair のみ freeze + 残りは active 維持で 19 本多 pair 相互作用に介入）。実装規模見積もり ~150 行（status-365 候補 (e) Phase 1 と同程度）。(2) Phase 1: Process 単体実装 + 単体テスト（`xkep_cae/contact/freeze/` 新設想定、`PairwiseFreezingProcess`）。(3) Phase 2: ContactFrictionProcess.freeze_slot 配線 + 19 本撚線実機検証（gate: frac ≥ 0.6）。(4) **却下時**: 失敗実験記録として残置し **(g2) AL 再導入**（status-221 で凍結した Uzawa 外側ループ 1〜2 回限定再導入）に進む。(5) 副次: 多 pair 診断 `14b_kc_multi_pair_diagnostic.py` 追加（status-370 §5 保留、(g3) 採択方向時に検証ライン補強）
