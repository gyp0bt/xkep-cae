...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7+10+12+11 テスト** — 2026-04-28 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-376（候補 (g2) AL 外側ループ限定再導入 + 19 本実機検証で却下、n=2 で +53.7% 改善も gate 0.026 不足で FAIL、候補 (g) 3 サブライン全終了）**。`HuberContactForceProcess` に `set_al_lambda_offset(λ)` / `get_last_p_n_eff()` API 追加で `p_n_eff = max(0, p_n_huber + λ)` を evaluate() で f_c に内包（K_geo は pair.state.p_n 経由で自動整合、modified Newton 補正不要 — 数理台帳 §9.2）。`NewtonDynamicProcess` の既存 NR while ループを `for _al_cycle in range(_al_n_uzawa_max):` 外側ループで包み、サイクル収束時に `λ_new = max(0, p_n_eff_converged)` で Uzawa 更新。法線成分のみ AL 適用（摩擦は status-147 NCP 鞍点系符号問題回避のため対象外）。実装: `strategy.py` +43 行 / `_newton_dynamic.py` +37 行（既存ループ全体を 4 spaces 字下げ）/ `core/data.py` + `process.py` + `StrandBendingOscillationConfig` 各 2 field（default OFF）+ 3 経路 plumb / `28_al_outer_loop_19strand.py` 新規 / `TestAugmentedLagrangianOffset` **11 単体テスト**追加。Default OFF 回帰: contact 468 + mathematics 109 + 11 AL = **588 passed 5 skipped** / `test_helical_3d_hermite` rel_err=2.18e-07 維持 / 7 本撚線 frac=1.0（8.43s）/ ruff + 全 24 契約検査 OK。数理台帳 `docs/math/03_huber_contact_penalty.md` §9 "Augmented Lagrangian motivation" 追記（+96 行、AL 定式化 / K_c 整合性 / status-221 凍結根拠 / status-376 動機 / status-147 摩擦符号問題）。**19 本撚線実機検証** (`28_al_outer_loop_19strand.py`) で `al_n_uzawa_max ∈ {2, 3}` 掃引: **n=2 frac=0.5746（baseline 0.3739 比 +53.7%、(g) サブライン全候補で最良 19 本実績）**、Gate `frac ≥ 0.6` を **0.026 不足で FAIL**。n=3 は 2 回目 Uzawa 更新で過修正発散（frac=0.1973、-47.2%、early abort）。λ 規模 ||λ||~6e-4〜9e-4、`n_λ_active=46/3590 pair`（~1.3% に局在）。**判定: 候補 (g2) 却下、候補 (g) 3 サブライン全終了**（(g1)+37.3% / (g3)-6.9% / (g2)+53.7% で gate 未達確定）。**結論**: NR alg 側 escape hatch は限界、19 本 Type D stall の主因 K_c x/z カップリング不整合（status-344 mat_only rel_err mean=44%）を直接解消しない限り frac=1.0 到達は困難。次候補: **explicit 時間積分**（`solver_mode="explicit"`）への移行 / 副次: K_mat の x/z 二次補正項追加（status-353/356 数理整理を覆すリスクあり保守的）。`al_outer_enabled=False` default 維持、`al_outer_enabled=True, al_n_uzawa_max=2` を 19 本以上向け opt-in escape hatch として運用。Phase A〜E / status-346〜376 の **27/N 完了**
...
- **status-377（次セッション・explicit 時間積分への移行）**: 候補 (g) 全終了確定を踏まえ、`StrandBendingOscillationConfig.solver_mode: Literal["implicit","explicit"]` 新設。explicit は dt 制限と引き換えに 19 本以上の K_c 構造的不整合を時間積分自体で安定化。設計参照: `phase_c3prime_19strand_plan.md` §4'。gate: 19 本 frac=1.0 完走（implicit + AL n=2 の 0.5746 を上回ること）。
