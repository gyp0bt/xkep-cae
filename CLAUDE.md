...
- **status-368（次セッション・候補 (d) 接触凍結モード 19 本再評価）**: (1) **最優先**: status-284 で 7 本 frac 0.40→0.70 を達成した `chattering_freeze_*` パラメータを 19 本 Type D stall 本体向けにチューニング、`freeze_max_cycles` / `freeze_nr_max` / `freeze_tol_factor` の感度掃引で MCDD 凍結解除条件（frac=1.0）達成可否を実測。(2) **副次**: 候補 (f) Phase C-3' s-tracking の 19 本再評価（status-355/356 の `K_hermite_adj` + `K_closest/K_st` active×adj 拡張）、active 変動領域の `_newton_dynamic.py` チューニング余地を探索。(3) 候補 (e) 7本 opt-in 活用提案: 3本/13本など他構成での有効範囲測定。(4) Phase E C25 候補: `@verified_by` challenge-test fixture 義務化
