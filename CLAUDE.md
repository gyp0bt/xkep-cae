...
**606 テスト** — 2026-04-03 | 契約違反 **0件** | 条例違反 **0件**
...
**接触あり90度曲げ frac=0.70→1.0** — status-284:
- 接触凍結モード（status-284）で frac=0.40→0.70（75%改善）
- frac=0.70付近で凍結サイクル5回でも残差不足 → 次の対策が必要
- 対策候補: Hertz型非線形ペナルティ（`p_n ∝ δ^{1.5}`）、凍結パラメータチューニング
