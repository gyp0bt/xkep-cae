...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7+10+12+11+28 テスト** — 2026-04-28 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-377（陽的中央差分時間積分 Phase 1 — `ExplicitCentralDifferenceProcess` 単体実装 + `solver_mode` config + 設計仕様、Phase 2 配線待機）**。候補 (g) 全終了確定（status-376）を踏まえ、19 本撚線 Type D stall に対する陰解法 NR 側 escape hatch 限界突破として陽解法時間積分への移行 Phase 1 を実施。`xkep_cae/time_integration/strategy.py` に `ExplicitCentralDifferenceProcess` 新設（+216 行、集中質量ロンピング row_sum/diagonal/none + 中央差分 `step()` API + Courant 臨界 dt + Verlet 予測子 + チェックポイント / `TimeIntegrationStrategy` Protocol 適合 4 メソッド）。`_create_time_integration_strategy()` に `solver_mode="explicit"` 分岐追加、`StrandBendingOscillationConfig.solver_mode: Literal["implicit","explicit"]` (default `"implicit"`) 追加、`solver_mode="explicit"` 実行時は `NotImplementedError` で Phase 2 待機を明示。設計仕様 `xkep_cae/time_integration/docs/time_integration_explicit.md` 新設（+126 行、数理定式化 / 集中質量化 / Courant 安定条件 / API / Phase 1/2 分割 / MCDD 脱法回避）。**Phase 1 制約**: Process 単体実装 + 単体テスト + 設計仕様のみ、solver path 配線は Phase 2（次 status）。Default `solver_mode="implicit"` で既存挙動完全不変、回帰: contracts 全 24 検査 OK / contact 468 + math 109 + time_integration 61 + strand_bending_oscillation 21 = **649 passed 5 skipped** / `test_helical_3d_hermite` rel_err=2.18e-07 維持 / 7 本撚線 frac=1.0 / ruff check + format pass。新規 28 単体テスト内訳: SDoF 自由振動 1 周期戻り誤差 < 5%、5 周期エネルギー有界 < 10%、Courant 越えで明確発散 100x、ロンピング row_sum/diagonal 数値検証、固定 DOF 拘束、減衰項 C·v 検証、対角質量 K_eff、`solver_mode` config 3 件。Phase 2 で `_explicit_dynamic.py` 新設 + Courant 監視 + adaptive Δt + 接触ペア再構築 + 19 本実機検証 gate `frac=1.0`（implicit + AL n=2 の 0.5746 上回り）。前 status: status-376（候補 (g2) AL 外側ループ限定再導入 + 19 本実機検証で却下、n=2 で +53.7% 改善も gate 0.026 不足で FAIL、候補 (g) 3 サブライン全終了）。Phase A〜E / status-346〜377 の **28/N 完了**
...
- **status-377 explicit 時間積分への移行（最優先、候補 (g) 全終了確定）** — status-376 で候補 (g) 3 サブライン全却下（(g1)+37.3% / (g3)-6.9% / (g2)+53.7% で gate 0.6 未達）が確定。NR alg 側 escape hatch アプローチは限界に到達したため、**陽解法時間積分への移行**が本命候補。設計参照: `xkep_cae/mathematics/docs/phase_c3prime_19strand_plan.md` §4' "solver_mode 併存方針"。実装方針: `StrandBendingOscillationConfig.solver_mode: Literal["implicit","explicit"]` 新設、explicit は dt 制限と引き換えに 19 本以上の K_c 構造的不整合を時間積分自体で安定化。gate: 19 本 frac=1.0 完走（implicit + AL n=2 の 0.5746 を上回ること）。
- **status-377 副次: K_mat の x/z 二次補正項追加** — 候補 (g) 全却下から「K_c 構造的不整合は active 集合変動下で NR alg 側 escape できない」が確定。MCDD 数理側の追加調査として K_mat の x/z 成分二次補正項（dp_n/du の二階微分寄与など）を再検討する選択肢があるが、status-353/356 で確立した K_geo ≡ K_mat,ndir 同一性の数理整理を覆す可能性があり保守的アプローチ。
- **多 pair 診断 `14b_kc_multi_pair_diagnostic.py` 追加** — status-370 §5 保留。explicit 移行と並行して 19 本領域の K_c 構造分析を補強。
...
- **status-378（次セッション・陽解法 Phase 2 solver path 配線 + 19 本実機検証）**: status-377 Phase 1 で完成した `ExplicitCentralDifferenceProcess` を `xkep_cae/contact/solver/` の専用 path に配線。`_explicit_dynamic.py` 新設（または `_newton_dynamic.py` への分岐追加、`NewtonDynamicProcess` と排他）、インクリメント単位で `step()` 駆動 + Courant 監視 + adaptive Δt（接線剛性最大固有値 power iteration + 0.9·Δt_c 安全運用）+ 接触ペア再構築 / 摩擦状態継承 / break-up 検知。`StrandBendingOscillationConfig.solver_mode="explicit"` 実行時の `NotImplementedError` ガードを削除し `_process_explicit()` 等専用パスへ分岐。gate: 19 本撚線 90° 曲げで `frac=1.0` 完走（implicit + AL n=2 の 0.5746 を上回ること）。設計参照: `xkep_cae/time_integration/docs/time_integration_explicit.md` §Phase 1 / Phase 2 分割。
