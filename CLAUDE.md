...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7 テスト** — 2026-04-24 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-369（Case B 19 本 opt-in ガイドライン化 + 候補 (f) Phase C-3' 実験計画 策定、documentation status）— status-368 §6 引継ぎに対応**。実装本体変更なしの documentation status として 2 成果を確定: (1) **TODO 2 副次 完了**: `chattering_freeze_nr_max=30` を 19 本以上向けの opt-in escape hatch として `StrandBendingOscillationConfig` docstring + `docs/roadmap.md` 「撚線規模別 opt-in チューニング」表に明記（4 項目 `chattering_freeze_nr_max` / `contact_damping_coefficient` / `smoothing_delta` / `contact_backtracking_*`、7 本既定 / 19 本推奨 / 実測効果 / 根拠 status）。(2) **TODO 1 reconnaissance 完了**: `xkep_cae/mathematics/docs/phase_c3prime_19strand_plan.md` を新設（+107 行）、候補 (f) Phase C-3' s-tracking 19 本再評価を **Step 3.1 active 境界 FD 診断**（~30 分、`14_kc_active_boundary_diagnostic.py` 新設、g=±10^-3 active 境界 perturbation で status-356 rel_err=2.18e-07 の境界悪化を検出）/ **Step 3.2 新項 `KcActiveFlipStiffness` 追加設計**（~2 時間、`TermExpansionContract` 6 項目化、`HuberContactForceProcess.tangent()` で Huber 2 階微分相当項を評価）に分割 scoping。MCDD 脱法パターン 1/4/5/6 回避チェックリスト + gate 基準（`test_helical_3d_hermite` rel_err <1e-5 維持 / 新 gate <1e-4 / 19 本 mat_only rel_err mean <0.25 / 19 本 frac≥0.8）明記。`docs/design/README.md` に plan doc 行を索引登録。次（status-370）は Step 3.1 実施 → 結果 A/B 判定 → Step 3.2 実装方針確定。Phase A〜E / status-346〜369 の **20/N 完了**（documentation status として計画進捗ゼロではなく次 status の実施可能性を拡張）**
...
- **status-370（次セッション・候補 (f) Phase C-3' Step 3.1 実施）**: (1) **最優先**: `phase_c3prime_19strand_plan.md` §3.1 に従い `work/beam_hysteresis/14_kc_active_boundary_diagnostic.py` を新設（`14_kc_closest_adj_diagnostic.py` を改造、g = ±10^-3 の active 境界シナリオを注入）。`test_helical_3d_hermite` シナリオで境界 perturbation 下の FD rel_err を測定し、status-356 の 2.18e-07 機械精度が境界で何桁悪化するかを数値化。(2) 結果 A（境界で rel_err 悪化）なら Step 3.2（新項 `KcActiveFlipStiffness` の `TermExpansionContract` 6 項目化設計）、結果 B（境界でも rel_err 健全）なら候補 (g) NR アルゴリズム側（active 判定履歴平滑化 / augmented-Lagrangian 再導入）に転換。(3) 副次: 既存 opt-in 表に他撚線数（3/13/37）測定値を埋める運用指針整備。(4) Phase E C25 候補は MCDD 本命優先で引き続き保留
