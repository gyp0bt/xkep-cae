...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7+10+12+11+28+10 テスト** — 2026-04-29 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-378（陽的中央差分 Phase 2 — solver path 配線 + Courant 監視 + 7 本 smoke test でスケーリング障壁 3×10⁵ 倍を実測、19 本 frac=1.0 は mass scaling 待機）**。status-377 Phase 1 の `ExplicitCentralDifferenceProcess` を `_explicit_dynamic.py` 新設で `ContactFrictionProcess` に配線。`ExplicitDynamicProcess` が `ContactForceAssemblyProcess` 経由で f_int + f_c を組み立て `step()` を 1 増分 1 step 駆動。Courant 監視は sparse Gerschgorin 上界、`dt_safe = 0.9·dt_c` で `dt_sub > dt_safe` 時に `failure_reason="courant"` で cutback 要求。`ContactFrictionInputData.solver_mode`/`explicit_courant_safety`/`explicit_courant_check_interval`/`explicit_mass_lumping` 4 field 追加、`StrandBendingOscillationConfig` 同 4 field を 3 経路 plumb、`NotImplementedError` ガード削除、`uses=[ExplicitDynamicProcess]` 追加、explicit モードでは `predict()` / `correct()` / `_u_pred` MPC 射影をスキップ。設計仕様 `xkep_cae/contact/solver/docs/explicit_dynamic.md` 新設。新規ユニット 10 件: API 4 / Courant 推定 4 / TypeError 1 / ContactFriction 統合 advance 1。**7 本撚線 90° 曲げ smoke test 実測**: `dt_sub=0.333` vs `dt_c=1.055e-06` で **Courant 比 3×10⁵**、3 回カットバックでも frac=0.0052 で打ち切り。Wiring（cutback 連携 / state 一貫性 / Courant 推定）は正常動作、19 本以上の K_c 主導 stiff 接触系に対し `mass scaling`（Belytschko §6.4.2） / `dt subcycling` が次 status の必須要件と確定。Default `solver_mode="implicit"` で既存挙動完全不変、回帰: 全 24 契約検査 OK / contact 478 + math 109 + time_integration 61 + strand_bending_oscillation 21 = **680 passed 5 skipped**（status-377 比 +31）/ `test_helical_3d_hermite` rel_err=2.18e-07 / 7 本 frac=1.0（implicit）/ ruff pass。前 status: status-377（陽的中央差分 Phase 1 — Process 単体実装 + 28 unit + 設計仕様）。Phase A〜E / status-346〜378 の **29/N 完了**
...
- **status-379（次セッション・陽解法 Phase 3: mass scaling / subcycling で 19 本 frac=1.0 ゲート挑戦）**: status-378 で実測した Courant 比 3×10⁵ を縮める数値手法を実装。候補 (h1) **mass scaling**（Belytschko §6.4.2、接触 DOF の質量を係数 β² 倍して dt_c を β 倍化、高周波モードのみ影響評価）/ (h2) **dt subcycling**（接触ペア陽解法 + 構造系陰解法の混合、`predict / correct` Protocol 適合は status-377 で確保済み）/ (h3) **selective explicit**（K_c x/z 不整合 DOF のみ陽解法駆動）。`ExplicitCentralDifferenceProcess` 側に質量倍化オプション追加 + `ExplicitDynamicProcess` の Courant 監視出力で β を逆算する API を追加。gate: 19 本撚線 90° 曲げで `frac=1.0` 完走（implicit + AL n=2 の 0.5746 を上回ること）+ mass scaling 影響を E_kin/E_strain 比で 5% 以内に抑制。設計参照: `xkep_cae/contact/solver/docs/explicit_dynamic.md` §既知のスケーリング障壁、status-378 §5。
