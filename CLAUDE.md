...
- テスト実行・ベンチマーク・長時間計算は**必ずファイルに tee** すること
- 例: `python -m pytest tests/ 2>&1 | tee /tmp/test-run-$(date +%s).log`
- バックグラウンド実行時も同様: `run_in_background` でもログファイルを確保
- パイプの `| tail -N` だけでは途中経過が確認できないため禁止
- 収束診断ログはリアルタイムで確認可能な形で出力する
...
- 全テスト共通で **16要素/ピッチ以上** を厳守すること（`min_elems_per_pitch=0` による密度チェックスキップは禁止）
...
**2271テスト（fast: 1691 + 1 xfailed / slow: 362 - 9 xfailed + deprecated: 218）** — 2026-03-07
