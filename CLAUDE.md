...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7+10(新) テスト** — 2026-04-25 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-371（候補 (g1) active 履歴 EMA 平滑化 実装）**。status-370 §5 最優先 TODO に対応、`HuberContactForceProcess` に `active_ema_alpha: float = 0.0` field 追加 + `_p_n_prev_array` 保有 + `reset_ema_state()` メソッド + `evaluate()` で `p_n_eff = α·p_n_new + (1-α)·p_n_prev` ブレンド（α=0 で履歴ストレージ未書き込みの byte-identical 動作維持）。`NewtonDynamicProcess.process()` の NR ループ突入時に `reset_ema_state()` を呼び出してインクリメント境界で履歴をクリア（責務分離）。4 層 1 field plumb-through（`_create_contact_force_strategy` / `default_strategies` / `ContactFrictionInputData` / `StrandBendingOscillationConfig`）+ 3 経路（曲げ / 揺動 / free_end）。`TestActiveEmaSmoothing` 10 単体テスト追加（default zero / factory plumb / first-iter raw / second-iter blend `α=0.3` / reset clears / α=1 identity / shape mismatch safe）。診断スクリプト `work/beam_hysteresis/26_active_ema_alpha_sweep.py` 新設（+150 行、`--n-strands {7,19}` × `--alphas` で α 掃引と gate 判定、7 本 frac=1.0 維持 / 19 本 frac≥0.6）。実機 α 掃引は **status-372 に分離**（status-365/366/367 と同じ Phase 1+2 構成、各 status は 1 PR 粒度を維持）。gate: 契約違反 **0 件**（全 24 検査 OK）/ 条例違反 **0 件** / `pytest xkep_cae/contact/` **456 passed 5 skipped**（baseline 446 → +10 EMA テスト）/ mathematics 109 passed / `test_helical_3d_hermite` rel_err=2.18e-07 維持 / 7 本撚線回帰 1 passed / ruff check + format pass。Phase A〜E / status-346〜371 の **22/N 完了**
...
- **status-372（次セッション・候補 (g1) α 掃引 実機検証）**: (1) **最優先**: `26_active_ema_alpha_sweep.py` で α ∈ {0.1, 0.3, 0.5} を 7 本 / 19 本撚線 90° 曲げで実測。gate: 7 本 frac=1.0 維持（status-336 baseline、未達ならロジックバグで revert）/ 19 本 frac ≥ 0.6（status-339 baseline 0.4839 比 +24%、未達なら (g3) に進む）/ `test_helical_3d_hermite` rel_err<1e-5 維持。(2) **採択方向の場合**: 19 本 frac ≥ 0.6 達成で `StrandBendingOscillationConfig.active_ema_alpha` の opt-in 推奨値を docstring + `docs/roadmap.md` の opt-in 表に追記（status-368/369 の `chattering_freeze_nr_max=30` と同パターン）。(3) **却下時**: 失敗実験記録として残置し (g3) pair-wise relaxation（status-284 接触凍結を pair granularity 拡張）に進む → (g2) AL 再導入。(4) 副次: 多 pair 診断 `14b_kc_multi_pair_diagnostic.py` 追加（status-370 §5 保留、(g1) 採択方向時に検証ライン補強）
