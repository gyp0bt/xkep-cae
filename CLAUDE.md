...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7+10+12 テスト** — 2026-04-25 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-375（候補 (g3) pair-wise relaxation Phase 2 NR 配線 + 19 本実機検証で却下）**。status-374 Phase 1 で完成した `PairwiseFreezingProcess` を NR ループ (`_newton_dynamic.py`) に配線し、`StrandBendingOscillationConfig` から 3 経路 plumb-through を整備。実装本体: `_newton_dynamic.py` +88 行（NR 反復先頭で is_active_now 構築 → flip_counts 更新 → process 呼出 → DOF block 上書きで f_c/R_u を snapshot 値に固定、既存 `chattering_freeze_*` 全体凍結は pairwise 有効時に排他抑止）/ `process.py` `freeze_slot = StrategySlot(default_types=(PairwiseFreezingProcess,))` 追加 + nr_config plumb / `core/data.py` + `_newton_dynamic.py` + `StrandBendingOscillationConfig` に各 3 field（`pairwise_freeze_enabled` / `_flip_threshold` / `_skip_type_d`、default OFF）。Default OFF 回帰: contact 468 passed 5 skipped / mathematics 109 passed / `test_helical_3d_hermite` rel_err=2.18e-07 維持 / 7 本撚線 90° 曲げ frac=1.0 完走（12.63s）/ ruff + 全 24 契約検査 OK。**19 本撚線実機検証**（`work/beam_hysteresis/27_pairwise_freeze_19strand.py`）で `flip_threshold ∈ {2, 3, 5}` 掃引、**全 3 ケース Gate `frac ≥ 0.6` 未達** で **候補 (g3) 却下**: t=2 frac=0.1975（baseline 0.3739 比 -47.2%）/ t=3 frac=0.3482（-6.9%）/ t=5 frac=0.1963（-47.5%）。pair-wise freeze 発動時 NR Type 分布 `A+B+D.div:71%` で divergence 加速、DOF block 上書きが隣接 pair に正のフィードバック誘発したと推定。19 本 Type D stall は K_c x/z カップリング不整合（status-344 mat_only rel_err 44%）が主因で active 集合振動の per-pair 凍結では解消できないと確定（候補 (c)/(d)/(e)/(g1) と同パターン）。次候補は **(g2) AL 再導入**（status-221 で凍結した Uzawa 外側ループ 1〜2 回限定再導入）。`pairwise_freeze_*` 3 field は default OFF のまま運用。Phase A〜E / status-346〜375 の **26/N 完了**
...
- **status-376 候補 (g2) AL 再導入** — status-375 で却下された (g3) を踏まえ、候補 (g) サブライン最後の (g2) Uzawa 外側ループ 1〜2 回限定再導入を実装。設計: status-221 で凍結した拡大ラグランジアン外側ループを `_newton_dynamic.py` の NR 内側 + AL 外側（max 2 cycle）の二重ループ化。摩擦接線剛性符号問題（status-147）の 19 本撚線への波及を再検証。gate: 19 本 frac ≥ 0.6（status-357 baseline 0.3739 比 +60%）。設計参照: `docs/math/03_huber_contact_penalty.md` §5 "AL motivation"。
- **status-376 副次（または別 status）** — `solver_mode` フラグ実装（陰解法 default / リスタート opt-in / explicit opt-in の 3 値拡張）。設計は `phase_c3prime_19strand_plan.md` §4' / `docs/roadmap.md` opt-in 表。リスタート側実装は更に別 status。
- **(g2) も却下時**: explicit 時間積分への移行（`solver_mode="explicit"` 拡張）を本格検討。陽解法は dt 制限と引き換えに 19 本以上の K_c 構造的不整合を時間積分自体で安定化。
- **多 pair 診断 `14b_kc_multi_pair_diagnostic.py` 追加** — status-370 §5 保留。(g2) 採択方向時に検証ライン補強。
...
- **status-376（次セッション・候補 (g2) AL 再導入）**: status-221 で凍結した Uzawa 外側ループ（拡大ラグランジアン）を 1〜2 回限定で再導入。`_newton_dynamic.py` の NR 内側 + AL 外側（max 2 cycle）の二重ループ化、摩擦接線剛性符号問題（status-147）の 19 本撚線への波及を再検証。gate: 19 本 frac ≥ 0.6。却下時は **explicit 時間積分**（`solver_mode="explicit"`）への移行検討。副次: `solver_mode` フラグ実装（陰解法 / リスタート / explicit の 3 値）
