...
**459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6+12+12+7+10+12 テスト** — 2026-04-25 | 契約違反 **0件** | 条例違反 **0件** | **MCDD status-374（候補 (g3) pair-wise relaxation Phase 1 — `PairwiseFreezingProcess` 単体実装）**。status-373 で確定した候補 (g3) ロードマップに対応、status-284 全体凍結モードを **pair granularity** に拡張する Phase 1 インフラを実装。`xkep_cae/contact/freeze/` サブパッケージを新規作成し（`strategy.py` 261 行 / `docs/pairwise_freezing.md` 159 行 / 12 単体テスト、合計 +664 行）、`PairwiseFreezingProcess` は per-pair active flip カウント + 現反復 active 状態 + グローバル chattering_type を入力に取り `pair_freeze_flags` + skip_freeze_global を返す純計算 Process。ヘルパ純関数 `_update_pair_active_flips` / `_is_type_d_dominant` は **C16 滅菌のため private 化**。NR 配線は Phase 2（status-375 以降）に分離（status-365 ContactNormalDamping と同 Phase 1/2 分割パターン）。実装本体（`_newton_dynamic.py` / `StrandBendingOscillationConfig` / `ContactFrictionProcess`）は **Phase 1 では無変更**。gate: 契約違反 **0 件**（全 24 検査 OK、C16 含む）/ 条例違反 **0 件** / `pytest xkep_cae/contact/` **468 passed 5 skipped**（status-373 比 +12 freeze テスト）/ mathematics 109 passed / `test_helical_3d_hermite` rel_err=2.18e-07 維持 / ruff check + format pass。Phase A〜E / status-346〜374 の **25/N 完了**
...
- **status-375 候補 (g3) Phase 2 NR 配線 + 19 本実機検証** — status-374 で実装した `PairwiseFreezingProcess` を `ContactFrictionProcess.freeze_slot` 経由で `_newton_dynamic.py` に配線し、既存全体凍結ロジック (`_freeze_active`/`_freeze_f_c`) を pair 単位拡張。`StrandBendingOscillationConfig` に 3 field 追加（`pairwise_freeze_enabled` / `_flip_threshold` / `_skip_type_d`）+ 3 経路 plumb-through。per-pair 力ベクトル組立は `HuberContactForceProcess` の per-pair 出力経路新設 or DOF ブロック上書き近似のいずれかで実装。検証: 7 本撚線 frac=1.0 維持回帰（既定 OFF でバイト一致）+ 19 本撚線 90° 曲げで `flip_threshold ∈ {2,3,5}` 掃引、gate **`frac ≥ 0.6`**（status-357 baseline 0.3739 の 60% 改善）。
- **status-375 副次** — `solver_mode` フラグ実装（陰解法 default / リスタート opt-in）。設計は `phase_c3prime_19strand_plan.md` §4' / `docs/roadmap.md` opt-in 表。リスタート側実装は更に別 status。
- **却下時の次候補**: 候補 (g3) gate 未達なら **(g2) AL 再導入**（status-221 で凍結した Uzawa 外側ループ 1〜2 回限定再導入）。それも却下なら **explicit 時間積分** への移行検討（`solver_mode="explicit"` 拡張）。
...
- **status-375（次セッション・候補 (g3) Phase 2 配線 + 19 本実機検証）**: `ContactFrictionProcess.freeze_slot` 配線 + `_newton_dynamic.py` pair 単位拡張 + `StrandBendingOscillationConfig` 3 field 追加 + 19 本撚線 90° 曲げ `flip_threshold` 掃引（gate `frac ≥ 0.6`）。副次: `solver_mode` フラグ実装。却下時は (g2) AL 再導入
