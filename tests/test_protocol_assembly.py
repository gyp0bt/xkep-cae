...
from pycae.elements.tri6 import Tri6PlaneStrain
from pycae.elements.quad4_bbar import Quad4BBarPlaneStrain
from pycae.materials.elastic import PlaneStrainElastic
from pycae.assembly import assemble_global_stiffness, assemble_global_stiffness_mixed
...
def test_generic_assembly_matches_legacy_quad4():
    ...
    K_new = assemble_global_stiffness(
        nodes, [(Quad4PlaneStrain(), conn)], mat, thickness=1.0, show_progress=False,
    )
    K_old = assemble_global_stiffness_mixed(
        nodes, conn, None, None, 10.0, 0.25, t=1.0, show_progress=False,
    )
...
def test_generic_assembly_matches_legacy_tri3():
    ...
    K_new = assemble_global_stiffness(
        nodes, [(Tri3PlaneStrain(), conn)], mat, thickness=1.0, show_progress=False,
    )
    K_old = assemble_global_stiffness_mixed(
        nodes, None, conn, None, 5.0, 0.3, t=1.0, show_progress=False,
    )
    assert np.allclose(K_new.toarray(), K_old.toarray(), atol=1e-10)
...
def test_generic_assembly_matches_legacy_tri6():
    ...
    K_new = assemble_global_stiffness(
        nodes, [(Tri6PlaneStrain(), conn)], mat, thickness=1.0, show_progress=False,
    )
    K_old = assemble_global_stiffness_mixed(
        nodes, None, None, conn, 200e3, 0.3, t=1.0, show_progress=False,
    )
    assert np.allclose(K_new.toarray(), K_old.toarray(), atol=1e-10)
...
def test_generic_assembly_mixed_solve():
    ...
    K = assemble_global_stiffness(
        ...
        mat, thickness=1.0, show_progress=False,
    )
