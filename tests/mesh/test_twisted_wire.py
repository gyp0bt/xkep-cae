...
class TestLargeScaleMesh:
    ...
    def test_61_strand_mesh(self):
        ...
        mesh = make_twisted_wire_mesh(61, _WIRE_D, _PITCH, _LENGTH, _N_ELEM)
        assert mesh.n_strands == 61
        assert mesh.n_nodes == 61 * (_N_ELEM + 1)
        assert mesh.n_elems == 61 * _N_ELEM

    def test_91_strand_mesh(self):
        """91本（1+6+12+18+24+30）メッシュ生成."""
        mesh = make_twisted_wire_mesh(91, _WIRE_D, _PITCH, _LENGTH, _N_ELEM)
