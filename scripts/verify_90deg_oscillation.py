"""7本撚線 90度曲げ+揺動の収束検証スクリプト.

SVD截断Schur complementで90度曲げ+揺動1周期を完遂する。

Usage:
  python scripts/verify_90deg_oscillation.py 2>&1 | tee /tmp/verify_90deg_osc.log
...
log_path = f"/tmp/verify_90deg_osc_{int(time.time())}.log"
...
print("=" * 70)
print("  7本撚線 90度曲げ+揺動1周期（UL+NCP, point contact, SVD截断）")
print("=" * 70)
...
if not result.phase1_converged:
    print("Phase1 FAIL")
    sys.exit(1)
if not result.phase2_converged:
    print("Phase2 FAIL")
    sys.exit(2)
print("ALL PASS")
