...
HEX8 要素ファミリ: C3D8（SRI+アワーグラス制御）、C3D8B（B-bar平均膨張法）、C3D8R（低減積分+アワーグラス制御）、C3D8I（非適合モード）。3D等方弾性テンソル。アセンブリ統合済み。Abaqus準拠命名。
...
- [実装状況](docs/status/status-062.md) — 最新のステータス（HEX8 アセンブリ統合 + B-bar 平均膨張法 + SRI アワーグラス制御）
