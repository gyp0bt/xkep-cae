...
| 項目 | 現状 | 目標 |
|------|------|------|
| 素線数 | **19本**（径方向圧縮で24ペアアクティブ接触）で収束 | 1000本（~30,000 DOF, 長手分割で~100,000節点） |
| 計算時間 | 91本で~25分/曲げ揺動 | 1000本で6時間以内 |
| ソルバー | NCP: **19本収束達成**、S3改良12項目（適応Δt/AMG/k_pen自動推定/残差スケーリング/接触力ランプ/初期貫入補正等）実装済み | NCP: 91本収束 |
| 接触ペア | 91本で~66,000候補 | 1000本で~730万候補→ML削減 |

## 現在の状態

**2122テスト** — 2026-03-05時点
...
| [ステータス一覧](docs/status/status-index.md) | 全112件のstatus + テスト数推移 |
