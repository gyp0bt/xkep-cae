"""Semi-smooth Newton ソルバー（NCP 鞍点系専用）.

NCP (Nonlinear Complementarity Problem) 鞍点定式化による接触解析。
smooth_penalty パスは solver_smooth_penalty.py に分離済み。
...
if TYPE_CHECKING:
    from xkep_cae_deprecated.process.strategies.protocols import LinearSolverStrategy

import numpy as np
import scipy.sparse as sp

from xkep_cae_deprecated.contact.assembly import (
    _contact_dofs,
    compute_contact_force,
    compute_contact_stiffness,
)
from xkep_cae_deprecated.contact.diagnostics import (
    ConvergenceDiagnostics,
    NCPSolveResult,
    NCPSolverInput,  # noqa: F401 — re-export for backward compatibility
)
from xkep_cae_deprecated.contact.graph import ContactGraphHistory, snapshot_contact_graph
from xkep_cae_deprecated.contact.initial_penetration import (
    adjust_initial_positions,
    check_initial_penetration,
)
from xkep_cae_deprecated.contact.law_friction import compute_mu_effective
from xkep_cae_deprecated.contact.mortar import (
    build_mortar_system,
    compute_mortar_contact_force,
    compute_mortar_p_n,
    identify_mortar_nodes,
)
from xkep_cae_deprecated.contact.pair import ContactManager, ContactStatus
from xkep_cae_deprecated.contact.staged_activation import (
    compute_active_layer_for_step,
    filter_pairs_by_layer,
)
from xkep_cae_deprecated.contact.utils import deformed_coords, ncp_line_search
...
def _deformed_coords(
    ...
    """参照座標 + 変位から変形座標を計算する（utils.deformed_coords の互換ラッパー）."""
    return deformed_coords(node_coords_ref, u, ndof_per_node)


# _ncp_line_search: utils.ncp_line_search からの互換エイリアス
def _ncp_line_search(
    ...
    """NCP Newton ステップの発散防止 line search（utils.ncp_line_search の互換ラッパー）."""
    return ncp_line_search(
        u,
        du,
        f_ext,
        fixed_dofs,
...
def _solve_linear_system(
    ...
    *,
    ...
    """
    from xkep_cae_deprecated.process.strategies.linear_solver import create_linear_solver
...
def _apply_bc_csr(K, rhs, fixed_dofs):
    ...
    from xkep_cae_deprecated.contact.bc_utils import apply_bc_fast
...
def newton_raphson_contact_ncp(
    ...
    *,
    ...
    if _penalty_strategy_pre is not None:
        ...
        from xkep_cae_deprecated.process.strategies.penalty import create_penalty_strategy
    ...
    if _friction_strategy_pre is not None:
        ...
        from xkep_cae_deprecated.process.strategies.friction import create_friction_strategy
    ...
    if _contact_force_strategy_pre is not None:
        ...
        from xkep_cae_deprecated.process.strategies.contact_force import (
    ...
    if _contact_geometry_strategy_pre is not None:
        ...
        from xkep_cae_deprecated.process.strategies.contact_geometry import (
    ...
    if _use_coating and _coating_strategy is None:
        from xkep_cae_deprecated.process.strategies.coating import create_coating_strategy
