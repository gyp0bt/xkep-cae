...
ロードパス:
  Phase 1（曲げ）: 端部回転角 θ を処方（変位制御）。adaptive_timestepping で自動分割。
  Phase 2（揺動）: z方向サイクル変位 ±Δz を n_cycles 周期
    - 変位制御: 自由端の z-DOF を拘束し、正弦波で処方
    - 曲げ角は固定維持

ソルバー: NCP Semi-smooth Newton（AL solver は status-167 で完全削除）。
...
from xkep_cae_deprecated.contact.pair import ContactConfig, ContactManager, ContactStatus
from xkep_cae_deprecated.contact.solver_ncp import newton_raphson_contact_ncp
from xkep_cae_deprecated.elements.beam_timo3d import ULCRBeamAssembler, assemble_cr_beam3d
from xkep_cae_deprecated.mesh.twisted_wire import TwistedWireMesh, make_twisted_wire_mesh
...
def export_bending_oscillation_gif(
    ...
    *,
    ...
    try:
        ...
        from xkep_cae_deprecated.output.render_beam_3d import VIEW_PRESETS, render_twisted_wire_3d
    except ImportError:
        return []
...
def run_bending_oscillation(
    ...
    *,
    ...
    if coating_thickness > 0.0:
        from xkep_cae_deprecated.mesh.twisted_wire import CoatingModel, coated_radii
