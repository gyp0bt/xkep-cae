...
メタクラス (ProcessMetaclass) との役割分担:
- メタクラス: process() ラップ + コールスタック追跡 + プロファイルデータ蓄積
- ProcessRunner: 依存チェック + 入力checksum + ログ出力 + dry_run
...
class ProcessRunner:
    ...
    def run_pipeline(
        ...
        各ステップの出力は次のステップには渡されない（独立実行）。
        パイプライン連鎖が必要な場合は BatchProcess を使用する。
        """
        results = []
