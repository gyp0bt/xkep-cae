"""7本ヘリカル素線 接触なし θ_y 90度曲げ 3Dパイプレンダリング.

status-281: 動的ソルバーでUL参照配置更新により完走。
変形形状をパイプレンダリングで物理的妥当性を確認する。

Usage:
    python contracts/visualize_7strand_bending_90deg.py
...
def main():
    print("Solving 7-strand helical 90deg bending (dynamic solver)...")
    ...
    # 先端変位表示
    _, right_nodes = _collect_end_nodes(conn, 7, sid)
    r_theory = 100.0 / (math.pi / 2.0)
    for s, rn in enumerate(right_nodes):
        tip = u[rn * 6 : rn * 6 + 6]
        print(f"  strand {s}: u_x={tip[0]:.2f}, u_z={tip[2]:.2f}, θ_y={math.degrees(tip[4]):.1f}°")
    print(f"  theory: u_x={r_theory:.2f}, u_z={-(100 - r_theory):.2f}")

    # --- 3Dパイプレンダリング ---
    fig = plt.figure(figsize=(18, 7))
    fig.suptitle(
        f"7-Strand Helical Wire 90° Bending (No Contact, Dynamic Solver)\n"
    ...
    # エラスティカ理論線
    theta_arr = np.linspace(0, math.pi / 2, 100)
    ...
    ax2.set_title("Deformed Configuration (90° bending)")
    ...
    plt.tight_layout()
    out_path = "docs/verification/7strand_90deg_bending_pipe.png"
