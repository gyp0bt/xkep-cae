...
4. **静的ソルバー警告**: `ContactFrictionProcess` が準静的パスを選択した際に `StaticSolverWarning` を送出
5. **レポート生成**: `docs/generated/process_usage_report.md` にプロセス使用レポートを自動出力
...
- `inspect.stack()` を使用することで、呼び出し側のコード変更なしに全プロセスの呼び出し元を追跡できる
- `atexit` フックにより、テスト/スクリプト終了時に自動的にレポートが生成される
- `ProcessRunner.run()` を経由しない直接の `process()` 呼び出しも追跡される（メタクラスレベルでラップ）
