"""ContactFrictionSolverProcess — 統一摩擦接触ソルバー.

NCPQuasiStaticContactFrictionProcess と NCPDynamicContactFrictionProcess を統合。
TimeIntegrationStrategy で準静的/動的の振る舞いを自動分離する。
...
- 時間積分 = SolverFrictionInputData.is_dynamic で自動選択
...
class ContactFrictionSolverProcess(SolverProcess[SolverFrictionInputData, SolverResultData]):
    """統一摩擦接触ソルバー（準静的/動的共用）.

    SolverFrictionInputData.is_dynamic に基づいて TimeIntegrationStrategy を
    自動選択する（準静的: QuasiStatic, 動的: Generalized-α）。

    Usage:
        solver = ContactFrictionSolverProcess()
    ...
    def process(self, input_data: SolverFrictionInputData) -> SolverResultData:
        """SolverFrictionInputData → solve_smooth_penalty_friction() → SolverResultData."""
        import time
        ...
        # 動的/準静的を is_dynamic で自動判定して Strategy を構築
        strategy_kwargs: dict = dict(
            ndof=ndof,
        ...
        strategies = default_strategies(**strategy_kwargs)
