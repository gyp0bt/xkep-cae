"""ContactFrictionSolverProcess の1:1テスト."""
...
@binds_to(ContactFrictionSolverProcess)
class TestContactFrictionSolverProcess:
    """ContactFrictionSolverProcess の単体テスト."""

    def test_meta(self):
        assert ContactFrictionSolverProcess.meta.name == "統一摩擦接触ソルバー"
        assert ContactFrictionSolverProcess.meta.module == "solve"
        assert not ContactFrictionSolverProcess.meta.deprecated

    def test_default_strategies(self):
        """デフォルト strategies でインスタンス化."""
        proc = ContactFrictionSolverProcess()
    ...
    def test_instance_dependency_tree(self):
        ...
        assert tree["name"] == "ContactFrictionSolverProcess"
        assert len(tree["uses"]) >= 3

    def test_registry_registered(self):
        from xkep_cae.process.base import AbstractProcess

        assert "ContactFrictionSolverProcess" in AbstractProcess._registry
