...
- SmoothPenaltyContactForceProcess: Huber型ペナルティ + Uzawa 拡大ラグランジアン
"""
...
class SmoothPenaltyContactForceProcess(
    ...
    """Huber型ペナルティ + Uzawa 拡大ラグランジアンによる接触力評価.

    接触力（Huber型 C¹ コンパクトサポート）:
        g ≥ 0:     p_n = 0                 （非接触、コンパクトサポート）
        -ε < g < 0: p_n = k_pen * g²/(2ε)   （C¹ 二次立ち上がり）
        g ≤ -ε:    p_n = k_pen * (-g - ε/2) （線形ペナルティ、full stiffness）

    εは smoothing_delta から導出: ε = 1/δ（softplus の遷移幅と同じ物理的意味）。
    Uzawa 更新: λ = max(0, λ + k_pen*(-g))

    特徴（softplus 比較、status-222）:
    - コンパクトサポート: g > 0 で力=0 → Uzawa 外ループ互換
    - C¹ 連続: 接触遷移でチャタリングなし（梁梁接触に最適）
    - g ≤ -ε で full stiffness → NR 2次収束
    ...
    @staticmethod
    def _huber_penalty(g: float, epsilon: float) -> float:
        """Huber型 C¹ コンパクトサポート接触ペナルティ.

        g ≥ 0:      p_n = 0                  （非接触）
        -ε < g < 0:  p_n = g²/(2ε)            （二次立ち上がり）
        g ≤ -ε:     p_n = -g - ε/2            （線形ペナルティ）

        ε = 0 の場合: max(0, -g) にフォールバック。
        """
        if g >= 0.0:
            return 0.0
        if epsilon <= 0.0:
            return -g
        if g > -epsilon:
            return g * g / (2.0 * epsilon)
        return -g - epsilon / 2.0

    @staticmethod
    def _huber_penalty_derivative(g: float, epsilon: float) -> float:
        """Huber型ペナルティの導関数: dp_n/dg.

        g ≥ 0:      dp/dg = 0
        -ε < g < 0:  dp/dg = g/ε              （線形に増加、g=0 で 0、g=-ε で -1）
        g ≤ -ε:     dp/dg = -1                （full stiffness）

        ε = 0 の場合: g < 0 で -1、g ≥ 0 で 0。
        """
        if g >= 0.0:
            return 0.0
        if epsilon <= 0.0:
    ...
    def evaluate(
        ...
        if hasattr(manager, "pairs"):
            for i, pair in enumerate(manager.pairs):
                ...
                # 拡大ラグランジアン: p_n = max(0, λ) + k_pen * φ(g)
                # Huber 型: コンパクトサポート（g ≥ 0 で φ=0）→ Uzawa 互換
                p_n = max(0.0, lam_i) + k_pen * self._huber_penalty(g_i, self._epsilon)
    ...
    def tangent(
        ...
        """接触接線剛性行列（Huber型 + Uzawa履歴、正定値）.

        f_c = -p_n * g_shape の接線:
            ∂f_c/∂u = -(dp_n/dg) * g_shape ⊗ g_shape

        重み決定ロジック:
            λ > 0:       weight = k_pen           （Uzawa 確認済みアクティブ: full）
            g ≥ 0:       weight = 0               （非接触）
            -ε < g < 0:  weight = k_pen * |g|/ε   （遷移帯: 線形に増加）
            g ≤ -ε:      weight = k_pen            （活性: full stiffness）

        λ > 0 のペアは Uzawa 外ループで「確実にアクティブ」と確認されている。
        遷移帯の弱い接線を使う必要がなく、full stiffness で NR 2次収束を実現。
        weight ≥ 0 → 常に正定値。
        """
        rows: list[int] = []
        cols: list[int] = []
        ...
        if hasattr(manager, "pairs"):
            for pair in manager.pairs:
                ...
                g_i = pair.state.gap
                deriv = self._huber_penalty_derivative(g_i, self._epsilon)
                if abs(deriv) < 1e-30:
                    continue

                # exact Huber tangent: weight = -k_pen * deriv ≥ 0（力と整合）
                # g ≤ -ε: weight = k_pen（full stiffness → NR 2次収束）
                # -ε < g < 0: weight = k_pen * |g|/ε（遷移帯）
                # δ を十分大きくして ε を小さくすれば操作点が線形領域に入る
                weight = -k_pen * deriv
                g_shape = _contact_shape_vector(pair)
                dofs = _contact_dofs(pair, self._ndof_per_node)
