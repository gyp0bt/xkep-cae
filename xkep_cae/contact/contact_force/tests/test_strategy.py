...
def _make_test_pair(
    *,
    ...
    return _evolve_pair(pair, state=state)
...
class TestNCPContactForceProcess:
    ...
    def test_evaluate_with_active_pair(self):
        proc = NCPContactForceProcess(ndof=24)
        pair = _make_test_pair(gap=-0.01)
        manager = _ContactManagerInput(pairs=[pair])
    ...
    def test_evaluate_inactive_pair_skipped(self):
        proc = NCPContactForceProcess(ndof=24)
        pair = _make_test_pair(gap=-0.01, status=_ContactStatus.INACTIVE)
        manager = _ContactManagerInput(pairs=[pair])
