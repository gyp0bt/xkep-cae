...
class NewtonDynamicInput:
    ...
    # `al_outer_enabled=True` のとき、内側 NR を `al_n_uzawa_max` 回繰り返し、
    # 各 NR 収束後に Uzawa 更新 `λ_new = max(0, p_n_eff_converged)` を per-pair で
    # 実施する。HuberContactForceProcess.set_al_lambda_offset(λ) を介して
    # `p_n_eff = max(0, p_n_huber + λ)` が f_c に寄与し、K_geo は pair.state.p_n
    # 経由で `p_n_eff` を読むため自動整合（K_mat は dp_n_huber/du のみで
    # dλ/du = 0 を反映、数理台帳 03_huber_contact_penalty.md §5 参照）。
    # 法線成分のみに AL を適用（摩擦は status-147 NCP 鞍点系の符号問題回避のため対象外）。
    # default OFF。19 本撚線 Type D stall に対する候補 (g) 最後のサブライン。
...
class NewtonDynamicProcess(
    ...
    def process(  # noqa: C901, PLR0912, PLR0915
        ...
        # === Augmented Lagrangian 外側ループ setup（status-376 候補 (g2)、status-221 凍結解除）===
        # `al_outer_enabled=True` のとき、内側 NR を `al_n_uzawa_max` 回繰り返し、
        # 各 NR 収束後に Uzawa 更新 `λ_new = max(0, p_n_eff_converged)` を per-pair で実施。
        # default OFF: `_al_n_uzawa_max = 1` で従来動作と等価（for は 1 回ループ）。
        # 法線成分のみ AL 適用（摩擦は status-147 NCP 鞍点系符号問題回避のため対象外）。
        # 数理台帳: docs/math/03_huber_contact_penalty.md §5 "Augmented Lagrangian motivation"
        _al_enabled = bool(cfg.al_outer_enabled)
        _al_n_uzawa_max = max(1, int(cfg.al_n_uzawa_max)) if _al_enabled else 1
        _n_pairs_total = int(len(manager.pairs)) if _al_enabled and hasattr(manager, "pairs") else 0
        _al_lambda_pairs: np.ndarray | None = (
            np.zeros(_n_pairs_total) if _al_enabled and _n_pairs_total > 0 else None
        )

        # === AL 外側ループ ===
        for _al_cycle in range(_al_n_uzawa_max):
            # ── AL: λ オフセット適用（None で完全無効化、default OFF）──
            if _al_enabled and hasattr(_contact_force_strategy, "set_al_lambda_offset"):
                _contact_force_strategy.set_al_lambda_offset(_al_lambda_pairs)
            ...
            while att + 1 < _effective_max:
                ...
                if _pairwise_enabled and _pwise_proc is not None and hasattr(manager, "pairs"):
                    ...
                    if diag.nr_iteration_snapshots:
                        _chatter_for_pwise = classify_chattering_type(
                            diag.nr_iteration_snapshots[-1]
                        )
                ...
                _conv_rate = (
                    diag.convergence_rate_history[-1] if diag.convergence_rate_history else 1.0
                )
                ...
                if (
                    ...
                    if _r_mean6 > cfg.tol_force * 10.0 and _ratio_spread < 100.0:
                        ...
                        _chatter_type = (
                            classify_chattering_type(_nr_snap)
                            if diag.nr_iteration_snapshots
                            else "-"
                        )
                ...
                if cfg.contact_backtracking_enabled and not _freeze_active:
                    ...
                    if _bt_trigger and hasattr(manager, "pairs"):
                        ...
                        def _compute_contact_metrics(
                            ...
                            _cres_tr = (
                                float(np.linalg.norm(_R_trial[_c_mask_tr]))
                                if _c_mask_tr.any()
                                else 0.0
                            )
                ...
                if att >= 4 and len(_u_history_ring) >= 3:
                    ...
                    for _period in range(2, len(_u_history_ring)):
                        ...
                        if abs(_idx) > len(_u_history_ring):
                            break
                        _cycle_diff = float(
                            np.linalg.norm(_u_history_ring[-1] - _u_history_ring[_idx])
                        )
                    ...
                    if _detected_period > 0:
                        ...
                        if _early_ok or _late_ok:
                            ...
                            _reason_tag = (
                                "微小接触"
                                if _early_ok
                                else f"後期リミットサイクル(R={_cur_ratio:.2f})"
                            )
                            if cfg.show_progress:
                                print(
                                    f"  Incr {increment_display} (frac={load_frac:.4f}), "
            ...
            # ── AL: NR 結果評価 + Uzawa 更新（status-376 候補 (g2)） ──
            # default OFF: `_al_n_uzawa_max=1` で `_al_cycle + 1 >= 1` 即 break、従来動作。
            # AL enabled: 内側 NR 収束時のみ追加サイクルへ。発散/nr_limit は外側で挽回不可。
            if not step_converged or _al_cycle + 1 >= _al_n_uzawa_max:
                break
            # Uzawa 更新: λ_new = max(0, p_n_eff_converged) per pair
            if (
                _al_enabled
                and _al_lambda_pairs is not None
                and hasattr(_contact_force_strategy, "get_last_p_n_eff")
            ):
                _last_p_n = _contact_force_strategy.get_last_p_n_eff()
                if _last_p_n is not None and _last_p_n.shape == _al_lambda_pairs.shape:
                    _al_lambda_pairs = np.maximum(0.0, _last_p_n.copy())
            if cfg.show_progress and _al_lambda_pairs is not None:
                _ln_al = float(np.linalg.norm(_al_lambda_pairs))
                _lmax_al = float(np.max(_al_lambda_pairs))
                _nact_al = int(np.sum(_al_lambda_pairs > 1e-30))
                print(
                    f"  [AL] Incr {increment_display} cycle "
                    f"{_al_cycle + 1}/{_al_n_uzawa_max} NR converged → Uzawa更新: "
                    f"||λ||={_ln_al:.3e}, max(λ)={_lmax_al:.3e}, "
                    f"n_λ_active={_nact_al}/{_n_pairs_total}"
                )

        # === AL 後処理: λ オフセットをクリア（次インクリメントへの干渉防止） ===
        if _al_enabled and hasattr(_contact_force_strategy, "set_al_lambda_offset"):
            _contact_force_strategy.set_al_lambda_offset(None)
