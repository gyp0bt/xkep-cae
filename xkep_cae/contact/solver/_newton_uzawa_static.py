"""Newton-Raphson + Uzawa イテレーション（純静的）.

NewtonUzawaDynamic と完全分離された静的版。
動的解析分岐（慣性力・減衰力・質量行列）を一切含まない。
内部ステップは全てサブプロセスに委譲。
"""
...
class NewtonUzawaStaticProcess(
    ...
    動的解析の慣性力・減衰力を含まない純静的版。
    内部ステップは全てサブプロセスに委譲する。
    """

    meta = ProcessMeta(
        name="NewtonUzawaStatic",
        module="solve",
        version="1.0.0",
    ...
    def process(  # noqa: C901, PLR0912, PLR0915
        ...
        n_uzawa_max = getattr(_contact_force_strategy, "n_uzawa_max", 1)
