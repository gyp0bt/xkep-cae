...
| leaf分類拡張 | **`dominant_leaf_process` 拡張（wrapper/leaf 自動分類）**: ProcessMetaclass に `_wrapper_classes` 追跡を追加し、`traced_process` の `_call_stack` を用いてランタイムで子を呼び出した Process を wrapper 判定。`get_profile_stats` / `BenchmarkRunnerProcess.profile_breakdown` / `ParameterSweepBenchmarkProcess.summary_rows` にそれぞれ `is_wrapper` / `dominant_leaf_process` を追加。status-316 知見 D（wrapper が dominant を占拠する問題）を解決。7 テスト追加 — status-317 |
| **次** | **100/200/500 本への掃引拡張（TangentAssembly 転換点特定、`dominant_leaf_process` で検証）+ 被膜 ON 掃引 + 被膜幾何接線剛性**（S6）／**ファイバー梁 Phase F1**（Elastic1D + BilinearKH 実装） → 1000本6時間 |
