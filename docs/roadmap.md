...
| C16修正+Hertz型 | **C16修正（RebuildMPCTransformProcess化）+ 凍結テスト + Hertz型非線形ペナルティ実装**: penalty_exponent=1.5でHertz接触。ベンチマーク未実施 — status-285 |
| **壁** | 接触frac=0.70→1.0（Hertz型ベンチマーク + パラメータ最適化）/ 61本以上 / 1000本6時間 |
