...
**Phase 1〜3 + Phase 4.1〜4.2 + Phase 5.1〜5.4 + Phase C0〜C5 + Phase C6-L1〜L5 + C6-L1b + Phase 4.7 Level 0 + L0.5 S1-S4 + ブロック前処理ソルバー + adaptive omega + Phase 6.0 PoC + ML基盤 + Phase S1 + Phase S2基盤 + Phase S2+（Broadphase強化 + 中点距離プリスクリーニング + S3ベンチマーク基盤）+ COOベクトル化 + 共有メモリ並列化 + 接触アセンブリnumpyベクトル化 + NR BC高速化 + 修正NR法 + S3タイミング計測基盤 + 撚線曲げ+サイクル変位ベンチマーク 完了。1882テスト（fast: 1541 / slow: 341）。**
...
- [ ] CR梁アセンブリCOO/CSR高速化（structural_tangent 80%ボトルネック）
- [ ] 19本撚り（3層）接触NR収束ベンチマーク — 収束性・貫入率・計算時間
- [ ] 37本撚り（4層）接触NR収束ベンチマーク
- [ ] 61本撚り（5層）接触NR収束ベンチマーク
- [ ] 91本撚り（6層）接触NR収束ベンチマーク
- [ ] 各段階で前処理パラメータチューニング（ILU drop_tol, Schur 対角近似精度）

**判断基準**: 91本で収束 + 計算時間が実用的（< 1時間/荷重ステップ）。

### S4: 剛性比較ベンチマーク（連続体 vs 素線被膜シース）

HEX8 レンガ要素の連続体モデルと、素線+被膜+シースモデルの剛性を比較。

- [ ] 同一断面形状の HEX8 連続体モデル構築
- [ ] 等価曲げ・ねじり・引張剛性の抽出
- [ ] 素線モデルとの定量比較（剛性マトリクス固有値）
- [ ] 結果に基づく**モデル選択指針**の策定

> ⚠️ この結果次第で後続の方針（ML ターゲット、1000本の手法）を調整する。
...
- [ ] HEX8 連続体 vs 素線被膜シース 剛性比較ベンチマーク（⚠️ 結果次第で方針調整）
- [ ] 接触プリスクリーニング GNN Step 2-5
- [ ] k_pen 推定 ML v2 Step 2-7
- [ ] 1000本撚線トライ（⚠️ 結果次第で GPU 化方針調整）
