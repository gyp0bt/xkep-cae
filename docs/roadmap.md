...
## 現在地（Phase C1 + FIELD ANIMATION出力 + .inpパーサー拡張 完了）

Phase 1〜3 + Phase 4.1〜4.2 + Phase 5.1〜5.4 + Phase C0〜C1 + 過渡応答出力 + FIELD ANIMATION出力完了（741テスト）。
...
| **I/O** | Abaqus .inp パーサー（*NODE/*ELEMENT/*NSET/**ELSET**/**BOUNDARY**/*BEAM SECTION/*TRANSVERSE SHEAR STIFFNESS/**OUTPUT, FIELD ANIMATION**）, CSV出力, Abaqusライクテキスト入力, **過渡応答出力（Step/Increment/Frame, CSV/JSON/VTK, バイナリ対応）**, **FIELD ANIMATION出力（梁2Dプロット, 要素セット色分け, xy/xz/yz 3ビュー）**, **run_transient_steps（ステップ列自動実行）**, **mesh_from_abaqus_inp（.inp→OutputDatabase変換）** |
...
| **検証** | 製造解テスト, Abaqusベンチマーク, 解析解比較, ロッキングテスト, 周波数応答解析解比較, Euler elastica, 弧長法, 弾塑性棒, ファイバーモデル曲げ, 過渡応答（SDOF/梁/集中質量）, 連続体非線形（TL/UL）, 非線形動解析, 動的三点曲げ, 陽解法, モーダル減衰, 接触幾何+broadphase+Active-set, 過渡応答出力+拡張, FIELD ANIMATION出力（**741テスト**）, [バリデーション文書](verification/validation.md) |
...
- 梁–梁接触モジュールは C0〜C1（データ構造+幾何+broadphase+Active-set）まで。法線接触力則・摩擦・ソルバー統合は C2〜C4 で実装予定
...
10. **Phase C2 法線AL + Active-setヒステリシス + 主項接線** — 次の実装候補
