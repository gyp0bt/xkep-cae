...
**~2260テスト + 275 新パッケージテスト** | 脱出ポット計画 Phase 6 進行中 — C14 強化 + ソルバー deprecated 依存除去（S3 凍結中） | [最新status](status/status-index.md)
...
- ✅ **脱出ポット計画 Phase 6（status-186）**: C14 強化 + ソルバー deprecated 依存除去
  ...
  - C14 違反 13件 → 4件に削減
- **脱出ポット計画 Phase 7〜8（未実施）**: deprecated 依存除去（mesh/contact/output + strategies 完全実装）
