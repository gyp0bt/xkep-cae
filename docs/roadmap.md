...
**~2260テスト + 315 新パッケージテスト** | 脱出ポット計画 Phase 12 完了 — Phase 13 調査完了（ビームアセンブラ移植が前提条件） | [最新status](status/status-index.md)
...
- ✅ **脱出ポット計画 Phase 12（status-195、完了）**: numerical_tests モジュール新 xkep_cae 移植
  ...
  - 70テスト全PASS
- **脱出ポット計画 Phase 13（次）**: ビームアセンブラ（CR/UL）の新 xkep_cae 移植
  - S3 xfail テストの Process API 対応の前提条件（status-196 で調査完了）
  - 移植対象: ULCRBeamAssembler + assemble_cr_beam3d + BeamSection（~1060行）
  - 移植先: `xkep_cae/elements/beam/` 新パッケージ
- **脱出ポット計画 Phase 14**: S3 xfail テストの Process API 対応版作成（Phase 13 完了後）
