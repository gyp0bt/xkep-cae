...
**~2260テスト + 275 新パッケージテスト** | 脱出ポット計画 Phase 5 完了 — ソルバー結果連携 + output クリーンアップ（S3 凍結中） | [最新status](status/status-index.md)
...
- ✅ **脱出ポット計画 Phase 5（status-185）**: ソルバー結果連携 + output re-export クリーンアップ
  ...
  - 4テスト追加（合計275テスト）
- **脱出ポット計画 Phase 6〜8（未実施）**: deprecated 依存除去（mesh/contact/solver）
