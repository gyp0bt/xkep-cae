...
**200+10s+16+3+23+1 テスト**（MPC+接触収束テスト frac=0.35到達 — status-254） | 契約違反**0件** | [最新status](status/status-index.md)
...
| MPC収束 | **MPC u伝搬修正 + NR内再射影 + 拡張系ラッパー**: frac=0.35到達（接触チャタリングで壁） — status-254 |
| **壁** | **MPC+接触のNR収束改善**（slave残差 + time integrator MPC未対応） / 61本以上 / 1000本6時間 |
