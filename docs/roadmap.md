...
**~2260テスト + 266 新パッケージテスト** | 脱出ポット計画 Phase 3 完了 — concrete プロセス移行（S3 凍結中） | [最新status](status/status-index.md)
...
- ✅ **脱出ポット計画 Phase 3（status-183）**: concrete プロセス移行（Mesh/Setup/Export/Render/Verify）
  ...
  - 62テスト追加（合計266テスト）
- **脱出ポット計画 Phase 4〜8（未実施）**: ContactFrictionProcess 移行 + 完全ワークフロー
