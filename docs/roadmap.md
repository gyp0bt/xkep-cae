...
## Phase S: スケーラビリティ — 1000本撚線への道

**目的**: 現在の7本撚り（~100 DOF）から1000本撚線（~102,000 DOF, 接触ペア 20,000-50,000）への段階的スケールアップ。

### 推奨ソルバー構成（リファレンス構成）

**今後の全ベンチマークは以下の構成を基準として実施する。**
ソルバー改良時は本構成表を更新し、status で変更点を明記すること。

| 項目 | 設定 | 根拠 |
|------|------|------|
| **ソルバー** | `newton_raphson_contact_ncp`（`solver_ncp.py`） | Outer loop 不要、λ暴走なし |
| **接触離散化** | Line-to-line Gauss 積分（`line_contact=True`） | セグメント間力の連続性 |
| **NCP関数** | Fischer-Burmeister（`ncp_type="fb"`） | 微分可能、正則化容易 |
| **摩擦** | Coulomb return mapping + NCP ハイブリッド（`use_friction=True`） | 法線NCP + 摩擦return mapping |
| **Mortar** | 適応ペナルティ付き Mortar（`use_mortar=True`） | 力の連続化 |
| **同層除外** | `exclude_same_layer=True` | ~80% ペア削減 |
| **k_pen** | 自動推定（beam EI ベース） | 手動設定不要 |
| **線形ソルバー** | DOF閾値自動切替（直接法 / GMRES+ILU） | スケーラビリティ |
| **前処理** | ブロック前処理 Schur 補集合（大規模時） | 接触+構造結合系の効率的解法 |

> **注**: AL法ソルバー（`solver_hooks.py`）はレガシー比較用として残すが、
> adaptive omega / lambda_n_max_factor 等のワークアラウンドが不要な NCP が標準。
> 詳細は [status-098](status/status-098.md) を参照。

> ⚠️ **10万要素超の想定**: MAX撚撚線モデルは長手方向にピッチ長分をモデル化する必要があり、
> 素線分割を細かくすると10万要素を容易に超える。要素アセンブリの高速化は
> この規模で直接効いてくるため、COOベクトル化・共有メモリ並列化を先行実装した（status-090）。

### S1: 接触高精度化（C6-L5 Mortar + 同層除外）
...
- [x] Broadphase グリッドビニング強化（セルインデックス一括計算 + バッチAABB重複判定, status-089）
- [x] 中点距離プリスクリーニング（Broadphase→Narrowphase間フィルタ, status-089）
- [x] Phase S3 スケーラビリティベンチマーク基盤（7/19/37/61/91本テスト, 26テスト, status-089）
...
- [x] S3タイミング計測基盤（BenchmarkTimingCollector, 工程別処理時間記録, 9テスト, status-092）

### S3: 撚線大規模化（91本まで）

段階的ベンチマーク: 19本 → 37本 → 61本 → 91本。

- [x] メッシュ生成・broadphaseスケーリングテスト基盤（7/19/37/61/91本, status-089）
- [x] 撚線曲げ+サイクル変位ベンチマーク基盤（90°曲げ + ±Δz 2周期, 7テスト, status-093）
- [x] CR梁アセンブリCOO/CSR高速化（status-096）
- [x] S3パラメータチューニング基盤: λ_nキャッピング + adaptive omega + ブロックソルバー（7本収束確認, status-097）
- [x] 19/37/61/91本収束レポートテスト（現状未収束 → ソルバー改良が必要, status-097）
- [ ] S3ベンチマークにNCPソルバー版を追加（リファレンス構成で再評価, status-098方針）
- [ ] 19本撚り（3層）接触NR収束達成 — NCPソルバー基準で評価
- [ ] 37本撚り（4層）接触NR収束達成
- [ ] 61/91本撚り接触NR収束達成
- [ ] 各段階で前処理パラメータチューニング（ILU drop_tol, Schur 対角近似精度）

**判断基準**: 91本で収束 + 計算時間が実用的（< 1時間/荷重ステップ）。リファレンスソルバー構成（NCP）で評価。

### S4: 剛性比較ベンチマーク（撚線構造レベル）

撚線構造（素線+被膜+シース）の等価剛性を、荷重モード別・大変形含めて系統的に比較する。
ベンチマークは常に**リファレンスソルバー構成（NCP）**で実施すること。

#### S4-0: 単体要素比較（完了・参考）

- [x] 同一断面形状の HEX8 連続体モデル構築
- [x] 等価曲げ・ねじり・引張剛性の抽出
- [x] 素線モデルとの定量比較（剛性マトリクス固有値）
- [x] 結果に基づく**モデル選択指針**の策定

**結果**: 梁モデルは円形断面特性を正確に反映。HEX8は矩形断面近似でねじり剛性に25%誤差。撚線解析には梁モデルが適切。
...
- [x] 素線+被膜 等価剛性ベンチマーク（曲げ/ねじり/引張/圧縮, 10テスト, status-098）
- [x] 素線+シース 等価剛性ベンチマーク（同上, status-098）
- [ ] 素線+被膜+シース フルモデル剛性ベンチマーク
- [ ] 大変形荷重-変位曲線の文献値比較

### S5: ML 導入（接触プリスクリーニング + k_pen 推定）

Phase 6.0 の GNN/PINN 基盤を接触問題に応用。

- [ ] 接触プリスクリーニング GNN Step 2-5（グラフ構築 → モデル → 推論統合 → 性能評価）
- [ ] k_pen 推定 ML v2 Step 2-7（残差ベースデータ生成 → 学習 → ContactConfig 統合）
- [ ] ハイブリッド GNN+PINN 組み合わせ検証

### S6: 1000本撚線トライ

S3 + S4 の結果を踏まえて挑戦。

- [x] メッシュ生成の拡張（1000本レイアウト）
- [x] broadphaseスケーリング計測（91→1000本: 0.12s→18s, ~733万候補ペア）
- [ ] メモリプロファイリング（K行列 nnz ~1.5M, 推定 ~2GB）
- [ ] 収束テスト（引張・曲げ・ねじり）← S3収束改善後
- [ ] 計算時間ベンチマーク

> ⚠️ ピッチ長モデル化で素線を長手方向に十分分割すると **10万要素超** になり得る。
> 要素アセンブリの COO ベクトル化 + 共有メモリ並列化（status-090）はこのスケールで効力を発揮する。

> ⚠️ この結果次第で GPU 化の優先度・手法を調整する。

### S7: GPU 対応

1000本の結果を踏まえ、ボトルネックに応じた GPU 化。

- [ ] プロファイリングに基づくホットスポット特定
- [ ] CuPy / JAX による疎行列演算の GPU 化
- [ ] GPU 対応反復ソルバー（GMRES on GPU）
- [ ] 要素行列計算の GPU バッチ化
...
S2 (CPU並列化)
  ↓
S3 (91本ベンチマーク)  ──→  結果次第で前処理チューニング
  ↓
S4 (剛性比較BM)       ──→  結果次第でモデル選択方針変更
  ├─ S5 (ML導入)      ──→  S4の結果でMLターゲット調整
  ↓
S6 (1000本トライ)     ──→  結果次第でGPU化の手法選定
  ↓
S7 (GPU対応)
```

後半（S4以降）は流動的。S4（剛性比較）と S6（1000本トライ）の結果で再度方針を調整する。
...
```
Phase 1-3 (アーキテクチャ, 梁要素, 幾何学的非線形) ✓
    ├── Phase 4.1-4.2 (弾塑性 + ファイバーモデル) ✓
    ├── Phase 5 (動的解析) ✓
    └── Phase C0-C5 (梁–梁接触基盤) ✓
            ├── Phase C6-L1〜L4, L1b (接触高精度化) ✓
            └── Phase 4.7 L0 + L0.5 S1-S4 (撚線基礎) ✓
                    ↓
            Phase S: スケーラビリティ  ← 現在地
              S1 (Mortar + 同層除外)
                ↓
              S2 (CPU並列化)
                ↓
              S3 (91本BM)  → S4 (剛性比較BM)
                               ↓
              S5 (ML) → S6 (1000本) → S7 (GPU)
```

**クリティカルパス**: S1 → S2 → S3 → S6（1000本撚線トライ）
...
> 完了済み TODO は最新 status に記録後、ここからは削除する。
...
### 中優先 — Phase S4-S6: 撚線構造剛性比較 & ML & 1000本

- [x] HEX8 連続体 vs 梁 単体要素剛性比較（梁モデルが適切と結論、status-097）
- [x] **素線+被膜 等価剛性BM**（曲げ/ねじり/引張/圧縮、NCPソルバー基準、10テスト、status-098）
- [x] **素線+シース 等価剛性BM**（同上、status-098）
- [ ] 素線+被膜+シース フルモデル剛性BM
- [ ] 大変形荷重-変位曲線の文献値比較（Costello, Foti & Martinelli）
- [x] 1000本撚線メッシュ生成・broadphaseスケーリング計測（status-097）
- [ ] 接触プリスクリーニング GNN Step 2-5
- [ ] k_pen 推定 ML v2 Step 2-7
- [ ] 1000本撚線接触NR収束テスト（S3収束改善後）

### 低優先 — Phase S7 & その他

- [ ] GPU 対応（CuPy/JAX 疎行列, GPU GMRES）
