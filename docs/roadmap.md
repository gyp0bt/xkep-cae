...
| `solver_mode`（status-377 Phase 1 実装、Phase 2 待機） | **"implicit"** | **"explicit"**（Phase 2 待機） | 陽的中央差分時間積分への切替。19 本以上の K_c x/z カップリング不整合（status-344 mat_only rel_err 44%）を時間積分自体で安定化する目的で status-377 Phase 1 に着手、`ExplicitCentralDifferenceProcess` 単体実装 + 設計仕様 + 28 単体テスト完了。**Phase 1 制約**: solver path 配線は Phase 2（次 status）で実施し、現状 `solver_mode="explicit"` 実行時は `NotImplementedError` で待機を明示。設計仕様: `xkep_cae/time_integration/docs/time_integration_explicit.md`。|
