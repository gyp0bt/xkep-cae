...
- ✅ **ContactManager Process 分割（status-205、完了）**: dataclass メソッド完全除去→Process 直接実装
  ...
  - 呼び出し元の整合は次PR対応
- ✅ **呼び出し元整合 + 旧テスト一掃（status-206、完了）**: 旧API→新API 145箇所移行 + 旧テスト89件削除
  - `_evolve()` → `_evolve_state()` / `_evolve_pair()` 全移行
  - `manager.detect_candidates()` 等 → Process API 全移行
  - C3/C5 契約違反解消（AddPair/ResetAllPairs テスト紐付け + uses 宣言修正）
  - 248テスト全PASS
