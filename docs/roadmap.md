...
| チャタリング内訳分析 | **NR反復レベル詳細診断（Type A/B/C/D/E分類）**: frac>0.10で活性集合振動ゼロ、接線剛性不整合(D=52%)が主因。凍結モードは原理的に無効 — status-287 |
| **壁** | 接線剛性(K_c+K_st)の不整合解消 → NR 2次収束回復 → frac=1.0 / 61本以上 / 1000本6時間 |
