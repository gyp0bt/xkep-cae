...
| **3D固体要素** | **HEX8 要素ファミリ — C3D8（SRI: 偏差=1点+体積=2×2×2）、C3D8R（低減積分+Flanagan-Belytschkoアワーグラス制御）、C3D8I（Wilson-Taylor非適合モード+静的縮合）。Abaqus準拠命名。** |
...
- 3次元連続体要素は HEX8 ファミリ（C3D8/C3D8R/C3D8I）を実装（シースバリデーション用、アセンブリ未統合）
