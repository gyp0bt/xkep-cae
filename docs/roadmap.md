...
| **3D固体要素** | **HEX8 要素ファミリ — C3D8（SRI+B-bar併用+アワーグラス制御: 偏差=1点SRI+B̄体積+HG制御）、C3D8B（B-bar平均膨張法: Hughes 1980、体積ロッキング回避）、C3D8R（低減積分+Flanagan-Belytschkoアワーグラス制御）、C3D8I（Wilson-Taylor非適合モード+静的縮合）。アセンブリ統合済み。Abaqus準拠命名。** |
| **材料** | 線形弾性（平面ひずみ）, **3D等方弾性（IsotropicElastic3D, 6×6 Voigt）**, 1D梁弾性 |
...
| **プロジェクト構成（thermal）** | `xkep_cae/thermal/` — `fem.py`（Q4熱伝導要素+アセンブリ+ソルバー+不規則メッシュ）、`dataset.py`（データセット生成+グラフ変換+不規則メッシュデータ）、`gnn.py`（メッシュGNNモデル+学習+評価）、`gnn_fc.py`（全結合GNN+ハイブリッドグラフ）、`pinn.py`（Physics-Informed学習）、`train_surrogate.py`（学習実行スクリプト） |

### 未実装（現状の制約）

- ラインサーチ・Lee's frame 等の追加ベンチマーク（Phase 3 オプション）
- 3次元連続体要素は HEX8 ファミリ（C3D8/C3D8B/C3D8R/C3D8I）を実装（アセンブリ統合済み）
...
- [x] 7本撚り収束改善 — AL乗数緩和(omega) + 反復ソルバー(GMRES+ILU) + Active Set Freeze + Pure Penalty方式で全3荷重ケース収束達成（9テスト、xfail解消）
...
- [x] 撚撚線（7本撚線＋被膜の7撚線）: 被膜込み接触半径・摩擦・断面剛性を用いた統合解析テスト（20テスト、3本撚線成功、7本xfail）
- [x] シース（外被）モデル（SheathModel, エンベロープ半径, 円筒管断面特性, 等価梁剛性, 最外層素線特定, 径方向ギャップ, 41テスト）
- [ ] シース挙動モデル — 解析的リングコンプライアンス + ペナルティ接触（下記 Stage S1〜S4）
- [x] 7本撚り収束達成 — Pure Penalty方式（n_outer_max=1, al_relaxation=0.01）+ Active Set Freeze + 反復ソルバー(GMRES+ILU)で引張/ねじり/曲げ全収束。pen_ratio=16%は将来的にadaptive omega or Mortar離散化で改善可能。（詳細: status-065）
...
- [x] **S1**: 均一厚リングコンプライアンス行列 — 厚肉リング理論 Fourier 展開閉形式（モード 0: 膜変形 c₀, モード n≥2: 曲げ cₙ）、N×N C 行列構築、FEM リング解との比較テスト（35テスト）
- [x] **S2**: 膜厚分布 t(θ) の Fourier 近似 — 素線配置からの内面形状計算（ray-circle交差）、Fourier 係数 aₙ/bₙ 抽出、膜厚分布考慮修正コンプライアンス行列（有効内径 a_eff Green関数）、高レベルAPI sheath_compliance_matrix()（34テスト）
...
### 6.0 2D定常熱伝導GNNサロゲート（原理検証PoC）✓

Phase 6 の先行実験として、2D定常熱伝導問題のGNNサロゲートモデルを構築。
FEMソルバー（xkep_cae/thermal/fem.py）でデータを生成し、GNNで温度場を予測する。

**問題設定**: 100mm×100mm アルミ板、10×10 Q4メッシュ、k=200 W/(mK)、h=25 W/(m²K)、
ランダム配置の矩形発熱体（15mm×15mm, q=5e6 W/m³, 1〜5個）。
予測対象: 全ノードの温度上昇 ΔT。

- [x] 2D定常熱伝導FEM（Q4フィン要素, 面対流+エッジ対流, 解析解検証, 12テスト）
- [x] データセット生成（ThermalProblemConfig, ランダム発熱体配置, FEM解, グラフ変換, 10テスト）
- [x] GNNサロゲートモデル（EdgeConvLayer, ThermalGNN, 学習/評価パイプライン, 7テスト）
- [x] 学習達成精度: **R²=0.973**, MAE=0.12°C, 相対誤差=8.56%（1000サンプル, 300エポック）

**得られた知見**:
- メッシュGNNの受容野制限（10×10グリッド ≈ 14ホップ対角 vs 10 MP層）が精度のボトルネック
- 物理的特徴量（熱ポテンシャル Σ1/(r²+ε)）の導入が決定的に有効（R²: 0.894→0.973）
- **全結合GNN（アテンション+距離カーネル）がメッシュGNNを上回る**: R²=0.995 vs 0.963
- **ハイブリッドGNN**（メッシュ+発熱ノードショートカット）: R²=0.979、エッジ数はメッシュの1.5倍

**追加検証結果（status-067）**:
- [x] サロゲートモデル追加検証（汎化性/ロバスト性/物理整合性/アブレーション/エネルギー収支, 9テスト）
- [x] 全結合GNN v2（FCAttentionLayer + 距離カーネル6D）→ **R²=0.995** (21テスト)
- [x] ハイブリッドGNN（メッシュ+発熱ノード長距離ショートカット）→ R²=0.979
- [x] 梁実装へのサロゲートモデル スポット導入レビュー（接触プリスクリーニング・k_pen推定が有望）

**PINN + 不規則メッシュ検証結果（status-068, 069）**:
- [x] Physics-Informed Loss導入（L_phys = ||K·ΔT-f||²、PINN学習モジュール、17テスト）
- [x] 不規則メッシュ（内部ノード摂動）でのGNN比較（11テスト）
- [x] PINN大規模メッシュ（20×20, 441ノード）検証 — R²>0達成、物理ロス減少確認（6テスト）
- [x] 不規則メッシュでのPINN効果検証 — 正則ΔR²+0.006 / 不規則ΔR²+0.021（11テスト）
- [x] 不規則メッシュ比較: FC GNN → ハイブリッドGNN統一（計算量O(N²)→O(N)削減）
- [x] 接触プリスクリーニングGNN設計仕様、k_pen推定MLモデル設計仕様（設計文書のみ、実装は撚線フェーズ次までペンディング）

**CI/テスト構成（status-070）**:
- [x] `@pytest.mark.slow` マーカー導入（283テスト: 接触・非線形・PINN・GNN比較）
- [x] GitHub Actions CI 3ジョブ構成（lint → test-fast → test-slow）
- [x] `[ml]` optional dependencies（torch, torch-geometric）
- [x] `claude/**` ブランチでの CI 自動実行

**次のステップ（TODO）**:
- [ ] 接触プリスクリーニングGNN Step 1 実装 — 撚線フェーズ次までペンディング
- [ ] k_pen推定MLモデル Step 1 実装 — 撚線フェーズ次までペンディング
...
Phase 3 (幾何学的非線形) ✓
    ...
    └── Phase C (梁–梁接触) ← Phase 3完了後に着手可
        ...
        Phase 4.7 (撚線モデル) ← Phase C + Cosserat rod + 非線形基盤
            ↓
Phase 6 (NNサロゲート) ← Phase 4の構成則設計に依存 ※ 6.0 熱伝導GNN PoC 完了 ✓
    ↓
