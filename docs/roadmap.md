...
**~2260テスト + 279 新パッケージテスト** | 脱出ポット計画 Phase 5 完了 — deprecated 完全除去準備 + 45° 曲げ収束確認（S3 凍結中） | [最新status](status/status-index.md)
...
- [設計仕様書](../xkep_cae_deprecated/process/docs/process-architecture.md)（status-150）
...
- ✅ **Phase 8（完了: status-164）**: ProcessRunner / StrategySlot / CompatibilityProcess / Preset
  - [設計文書](../xkep_cae_deprecated/process/docs/phase8-design.md)
...
- ✅ **脱出ポット計画 Phase 1（status-175）**: xkep_cae → xkep_cae_deprecated リネーム + PenaltyStrategy 完全書き直し
...
- ✅ **脱出ポット計画 Phase 4（status-184）**: ContactFrictionProcess 移行 + StrandBendingBatchProcess v3.0.0 完全ワークフロー
  ...
  - 62テスト追加（合計266テスト）
- ✅ **脱出ポット計画 Phase 5（status-185）**: deprecated 完全除去準備
  - 全モジュール xkep_cae_deprecated → xkep_cae 移植完了
  - 新規手書き: math/quaternion, core/constitutive, core/element, core/results, core/state, sections/beam
  - re-export shim 全廃止（9パッケージ __init__.py 書き換え）
  - 7本撚線 45° 曲げ deprecated 無しで収束確認
- **脱出ポット計画 Phase 6〜8（未実施）**: sed 移植ファイルの C16 契約違反対応 + deprecated ディレクトリ完全削除
...
> 詳細: [status-index](status/status-index.md) | 各モジュール README: [contact](../xkep_cae_deprecated/contact/docs/README.md) / [elements](../xkep_cae_deprecated/elements/docs/README.md) / [mesh](../xkep_cae_deprecated/mesh/docs/README.md)
