...
**~2260テスト + 356 processテスト** | R1 ContactFrictionProcess統合+executor NCP版（S3 凍結中） | [最新status](status/status-index.md)
...
- ✅ **NCP ソルバーリファクタリング（status-168）**: solver_ncp.py 分離 + Process 移行
  ...
  - NCPDynamicContactFrictionProcess / NCPQuasiStaticContactFrictionProcess 新設
