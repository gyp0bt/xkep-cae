...
**~2260テスト + 204 新パッケージテスト** | 脱出ポット計画 Phase 2 完了 + C16スコープ拡大 + process/削除（S3 凍結中） | [最新status](status/status-index.md)
...
- ✅ **Penalty/Coating ファクトリ完備（status-181）**: `_create_penalty_strategy` + Coating パッケージ移行
  ...
  - 204テスト
- **脱出ポット計画 Phase 3〜8（未実施）**: concrete プロセス移行（Mesh/Setup/Export/Verify）
