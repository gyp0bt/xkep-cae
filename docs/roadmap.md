...
| **次** | **K_st の TangentAssembly からの分離計測**／**ContactForceStStiffness/FrictionStStiffness の n² 成長抑制**（空間ブロック分離・距離カット・ML 削減）／**status-299/301-equivalent proven setup での n=7/19/37 再測定**（収束を保証してスケール純度を高める）／**`uses` グラフ拡張（StrategySlot uses 宣言）+ 被膜 ON 掃引**／**ファイバー梁 Phase F1**（Elastic1D + BilinearKH 実装）／**pypardiso 環境 6 ケース再ベンチ** → 1000本6時間 |
