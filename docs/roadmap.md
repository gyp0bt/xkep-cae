...
| **プロジェクト構成（thermal）** | `xkep_cae/thermal/` — `fem.py`（Q4熱伝導要素+アセンブリ+ソルバー）、`dataset.py`（データセット生成+グラフ変換）、`gnn.py`（GNNモデル+学習+評価）、`train_surrogate.py`（学習実行スクリプト） |
