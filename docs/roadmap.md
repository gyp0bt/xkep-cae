...
| **次** | **19本撚線 Type D 対策（status-343 新設 Process を solver フックに組み込み、mat/geo/st 由来切り分け → 仮説 A 検証、frac=1.0 完走）**／**7本撚線ピッチ依存性検証（p=50/100/200）**／**揺動サイクル履歴依存性観測**／**ファイバー梁キャリブレーション（7本の mean κ_cr + CV=0.30）**／**空間ブロック分離 or ペアクラスタリング**（n² 根本対策）→ 1000本6時間 |
