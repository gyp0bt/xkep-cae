...
| プロファイルAPI | **ProcessMetaclass構造化プロファイル統計API強化 + BenchmarkRunnerへのprofile_breakdown自動キャプチャ**: snapshot_profile/get_profile_stats/get_profile_report（sort_by/top_n/since）+ RunManifestへYAML出力。13テスト追加 — status-313 |
| **次** | **1000本撚線プロファイリング実測 + 被膜幾何接線剛性** → 1000本6時間 |
