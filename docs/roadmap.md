...
**次のステップ（TODO）**:
- [ ] Physics-Informed Loss（熱伝導方程式残差）の効果検証
- [ ] 不規則メッシュでの全結合GNN vs メッシュGNN再検証
