...
**~2260テスト + 315 新パッケージテスト** | Phase 16 完了 — C16/C17 違反ゼロ + API 整合完了 | [最新status](status/status-index.md)
...
- ✅ **ContactManager Process 分割（status-205、完了）**: dataclass メソッド完全除去→Process 直接実装
  ...
  - 呼び出し元の整合は status-206 で完了
- ✅ **呼び出し元 API 整合（status-206、完了）**: 旧 dataclass メソッド → 新 API 完全移行
  - `_evolve()` → `_evolve_state()` / `_evolve_pair()` : 11箇所
  - `manager.detect_candidates()` → `DetectCandidatesProcess` : 35箇所
  - `manager.update_geometry()` → `UpdateGeometryProcess` : 14箇所
  - `manager.add_pair()` → `AddPairProcess` : 15箇所
  - 315テスト + 29テスト全PASS
