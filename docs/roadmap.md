...
| **シース接触力学** | 解析的リングコンプライアンス | 追加DOFゼロ、厚肉リング理論でせん断含む、Fourier平滑化で凹包不連続回避 |
| **シース内面形状** | Fourier近似による凹包 | ポリノミナル凹包を平滑化、膜厚分布 t(θ) をモード係数に反映 |
| **シース-シース接触** | 円-円接触（既存梁-梁フレームワーク） | 撚撚線の外側シース同士は円形断面として既存接触基盤を再利用 |
...
- [ ] **シース接触力学 — 解析的リングコンプライアンス方式** (後述)
- [ ] 7本撚りブロック分解ソルバー（Schur補完法 or Uzawa法、36+ペア同時NR収束の根本解決）

#### シース接触力学 — 解析的リングコンプライアンス方式

**課題**: シース内面は素線配置に沿う非円形形状（ポリノミナル凹包）、外面は円形 → 周方向に膜厚分布が生じる。素線食い込み（強接触）も表現したいが、連続体要素は計算コスト大、シェル/梁要素ではせん断自由度不足でロッキング、凹包は接触不連続性で不安定。

**方針**: シースを FE 離散化せず、**厚肉弾性リングの解析解**で径方向コンプライアンスを表現する。追加 DOF ゼロ、ロッキング不在、せん断変形自動包含。

**原理**:
- z=const 断面でシースを厚肉弾性リングとして扱う
- N 本の最外層素線からの径方向集中荷重を受けるリングの変形を Fourier 級数で解析的に解く
  - 7本撚り（外層6本）: 励起モード n = 0, 6, 12, ...
  - 19本撚り（外層12本）: 励起モード n = 0, 12, 24, ...
- **N×N コンプライアンス行列 C** を構築: `δr = C · F_contact`
  - モード 0（膜変形）: 均等圧縮
  - モード n≥2（曲げ）: 素線間の局所凹み
  - C の非対角項で接触点間の周方向カップリングを表現

**凹包の平滑化**:
- 内面形状を Fourier 近似: `r_inner(θ) = R₀ + Σ aₙ cos(nθ) + bₙ sin(nθ)`
- 多角形凹包の不連続を Fourier 打切りで自然に平滑化
- 膜厚分布 `t(θ) = R_outer - r_inner(θ)` を各モードの剛性に反映

**有限滑り**:
- 素線接触位置 θ_contact を追跡、変形に伴い移動
- θ 変化に応じて C 行列の接触点列を再配置
- 摩擦は既存 `friction_return_mapping` を使用（径方向=法線、周方向+軸方向=接線）

**シース-シース接触**:
- 撚撚線: 外側シース外面（円形）同士の接触 → 円-円接触
- 既存の梁-梁接触フレームワーク（2つの円形断面の最近接点計算）がそのまま適用可能

**段階的実装**:
- [ ] Stage 1: 均一厚リングの解析的コンプライアンス行列（モード 0 + n の閉形式、FEM リング解との比較検証）
- [ ] Stage 2: 膜厚分布 t(θ) の Fourier 導入 + 修正コンプライアンス
