...
**ソルバーパス**: 全テストで `ContactFrictionProcess → NewtonUzawaStaticProcess`（準静的）を使用。
mass_matrix / dt_physical 未指定のため動的パス（`NewtonUzawaDynamicProcess`）は未検証。
動的ソルバーのテストは別途作成が必要。
...
| TestSevenStrandRadialProcessAPI | 1 | CFP→NUStatic | 7本撚線の線形梁径方向圧縮 |
| TestSevenStrandBendingProcessAPI | 2 | CFP→NUStatic | 7本 UL CR 梁 45°/90° 曲げ |
| TestSevenStrandBendingOscillationProcess | 1 | CFP→NUStatic | 7本 曲げ + 揺動 (xfail) |
| TestStrandFrictionProcess | 1 | CFP→NUStatic | 7本 CR梁摩擦曲げ (xfail) |
| TestLargeStrandProcessAPI | 1 | CFP→NUStatic | 19本径方向圧縮 (xfail: CI timeout) |
| TestStrandBendingPhysicsProcess | 2 | CFP→NUStatic | 物理検証（先端変位方向 + 法線力正値） |

> CFP = ContactFrictionProcess, NUStatic = NewtonUzawaStaticProcess
...
- [ ] **動的ソルバーテスト作成**: CFP→NewtonUzawaDynamicProcess パスの検証テスト（mass_matrix + dt_physical 指定）
...
- **ソルバーパスの明記を徹底すること**: ContactFrictionProcess は mass_matrix/dt_physical の有無で準静的（NUStatic）と動的（NUDynamic）を自動切替する。テスト成果やドキュメントには必ず使用ソルバーパスを明記する。混同は混乱の元になる
...
- ULCRBeamAssembler + ContactFrictionProcess（準静的パス）の統合が初めてテストで検証された
