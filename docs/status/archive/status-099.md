...
> 本statusはPR#83でmasterにマージされた内容。status-097と番号が衝突したため099に再番付与。
...
1. **S3パラメータチューニング**: n_outer_max 増加やステップ数増加で収束性改善の検討 → status-097で実施済み
2. **1000本撚線ベンチマーク**: geometry_update の O(n²) スケーリング対策が必要
3. **S4: 剛性比較ベンチマーク** → status-097, 098で実施済み
4. **xfail テストの根本対策**（n_outer_max=5 安定化、摩擦付きヒステリシス） → status-097で実施済み
