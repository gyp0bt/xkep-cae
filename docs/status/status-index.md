...
| [362](status-362.md) | 2026-04-22 | **仮説 C 候補 (c) — `ContactBacktrackingLineSearchProcess` 実装**: status-361 Type 分布所見（19本重荷重のみ mixed (C+D) 16.6% 突出）への直接対策として backtracking line search を NR 主ループに追加。`ContactBacktrackingLineSearchProcess` 新設（`_newton_steps.py` +112 行）が既存 `NCPLineSearch` の ||R_u|| 全体発散判定では捉えられない接触残差比 / active flip 過剰増加を検知し α を半減する。トリガー条件 `att≥2 & n_active≥1 & active_set_changed & _conv_rate>rate_threshold` で mixed 狭義検知、`active_flip_threshold` は `max(abs=3, ratio=0.3 × n_active_pre)` の相対判定。`NewtonDynamicInput` / `ContactFrictionInputData` / `StrandBendingOscillationConfig` / `ContactFrictionProcess` の 4 層で 9 field を整備、**default OFF で既存動作不変**。`TestContactBacktrackingLineSearchProcessAPI` 6 テスト（6 新規）+ default OFF regression `pytest xkep_cae/contact/solver/ xkep_cae/numerical_tests/` 163 passed 6 skipped 1 xfailed で回帰なし。`19_hypothesis_c_backtracking_7strand.py` + `20_hypothesis_c_backtracking_19strand.py` 新設で opt-in 実測検証。gate: 契約違反 **0 件** / 条例違反 **0 件** / ruff check + format pass。Phase A〜E / status-346〜362 の **14/N 完了** | 459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6 passed |
...
仮説 C (c) ContactBacktrackingLineSearchProcess 実装: 459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25+6(新) (2026-04-22) ← status-362（status-361 Type 分布所見「mixed (C+D) 16.6%」への直接対策として仮説 C (c) 実装。`ContactBacktrackingLineSearchProcess` 新設（`_newton_steps.py` +112 行）で既存 `NCPLineSearch` の ||R_u|| 全体発散判定では捉えられない接触残差比 / active flip 過剰増加を検知し α 半減。`_newton_dynamic.py` NR 主ループに組込、トリガー条件は `att≥2 & n_active≥1 & active_set_changed & _conv_rate>rate_threshold` で mixed 狭義検知。active_flip_threshold は `max(abs=3, ratio=0.3 × n_active_pre)` の相対判定。`NewtonDynamicInput` / `ContactFrictionInputData` / `StrandBendingOscillationConfig` / `ContactFrictionProcess` で 9 field × 4 層を整備、default OFF で既存動作不変。`TestContactBacktrackingLineSearchProcessAPI` 6 テスト追加、solver tests 109 passed 5 skipped、default OFF regression 163 passed 6 skipped 1 xfailed、契約 0 違反 / 条例 0 違反。`19_hypothesis_c_backtracking_7strand.py` + `20_hypothesis_c_backtracking_19strand.py` 新設で 7/19 本実測検証、計画 14/N 完了）
```
