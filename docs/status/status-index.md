...
| [354](status-354.md) | 2026-04-19 | **7本撚線 90度曲げ（接触あり）重量回帰確認 — frac=1.0 維持 + 38% 高速化**: status-353 数理台帳訂正後の重量回帰を実施。`contracts/verify_7strand_90deg_contact.py`（status-326 `1e3b930` で掃除された `verify_s_unclamped_90deg.py` を git history から再整備）で κ=π/2/L=0.01571 [1/mm] (bending_angle=90°)、Hertz 型 α=1.5、free_end_mode=True、contact_enabled=True、μ=0.15 の status-298 同一設定を実行。**frac=1.0000, incr=542, cutback=52, 465.0s** 完走。status-298 baseline (`frac=1.0000, incr=535, cutback=45, 752s`) に対し **-287s (-38.2%)** 高速化（status-308〜326 の K_st vectorize / cKDTree / distance culling / symbolic factor reuse の累積効果）。cutback 内訳: `diverged` 8 (初期接触活性化)、`nr_limit` **43 (82.7%、frac 0.42〜0.97 散在)**、`relax_fail` 1。収束型統計 force=19 (3%)、disp=490 (92%)、energy=20 (3%) — disp 偏重は status-298 時点からの構造的特徴。エネルギー収支: SE 0.344→1.556 (4.5倍)、KE 0.0119→0.0028 (減衰率 4.38)、物理的に妥当。status-353 の「⚠️ 接触あり 90° 曲げ回帰は本 status 未実行」を解消。`nr_limit` 主体の cutback 分布は status-353 仮説 A（`K_hermite_adj` mat-only）と整合、Phase C-3 再定義（`I_nn` 隣接拡張）で改善見込み。実装変更なし、既存テスト数変動なし | 459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25 passed |
...
MCDD Phase C-2:         459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25(新) (2026-04-18) ← status-351（`KcHermiteNonlocal` / `KcClosestPoint` Process 抽出、5 項 `TermExpansionContract`、11 テスト追加、7本撚線 frac=1.0 回帰合格、計画 6/11 完了）
中断スナップショット:   459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25(維) (2026-04-19) ← status-352（計画書 `deep-wiggling-seal.md` 永久ロスト記録 + Phase C-3 前提の数理検証、`K_geo` が既に `−p_n · ∂n̂/∂u` と同型のテンソルを返すことを確認、実装は次セッションに引き継ぎ、テスト数変動なし）
数理台帳訂正:           459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25(維) (2026-04-19) ← status-353（`docs/math/03_huber_contact_penalty.md` §3/§3.1/§4/§5/§8 訂正、`K_mat,ndir ≡ K_geo` 同一性確立、当初 Phase C-3 撤回 + 5 項完結化、Phase C-3 再定義 = `K_hermite_adj` フル項拡張、テスト数変動なし）
7本撚線90度回帰:        459+13+22+5+8+12+12+25+26+10+15+10+9+8+12+33+33+21+8+25(維) (2026-04-19) ← status-354（7本撚線 90度曲げ接触あり重量回帰確認、`frac=1.0000, incr=542, cutback=52, 465.0s` で完走、status-298 baseline 752s 比 -38% 高速化、cutback +7 は `nr_limit` 主体で `K_hermite_adj` mat-only 仮説と整合、テスト数変動なし）
```
