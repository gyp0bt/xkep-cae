# status-278: 7本撚線NR収束壁の多層的原因特定 — 回転残差+微小接触フィルタ+dm一貫化
...
|--------|--------------|-------------|------|
| 2本撚線 | 0.64 | **0.70** | +0.06 |
| 7本撚線 | 0.38 | **0.55** | +0.17 (46%改善、DOF固定は追加効果なし) |
...
NR停滞時の回転残差の91-97%は**θ_z（曲げ面内回転）**。端部隣接ノード（各ストランドの2番目のノード）で ±1.7〜2.0 の固定値。

| 成分 | NR反復後の比率 | 備考 |
|------|-------------|------|
| θ_x | 20-30% | NR反復で改善される |
| θ_y | 20-30% | NR反復で改善される |
| **θ_z** | **91-97%** | **NR反復で不変** |

**原因**: lumped質量行列の回転慣性 `m*L²/78 = 2.2e-8` が極小で、動的有効剛性の慣性項寄与がゼロ同然（c0*M_rz = 7.4e-7 vs K_rot = 4084）。ただし K_rot（構造回転剛性）が支配的なため NR更新量は適切（du_rz ≈ 5e-4 rad）。**θ_z残差はインクリメントごとに単調増加（-1.72→-2.0）する内力残差**で、NR反復では変化しない。

### 7. 変位収束の並進限定

変位収束判定を並進DOFのみに変更。ただし7本撚線では追加効果なし（frac=0.5543不変）。接触2サイクル振動で並進duも振動するため。
...
| テスト | ベースライン | 修正後 | 主因 |
|--------|------------|--------|------|
| 2本撚線 | 0.64 | **1.00（完走）** | 参照点θ_x,θ_y固定が決定的 |
| 7本撚線 | 0.38 | **0.55** | 微小接触フィルタで+0.17 |
