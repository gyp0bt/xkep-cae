...
- [ ] 撚撚線（7本撚線＋被膜の7撚線）: 被膜付き7本撚線の接触解析テスト。被膜込み半径・摩擦係数・断面剛性を統合した統合テスト
- [ ] シース被膜対応（撚線全体を覆う外被）: 現在の CoatingModel は素線個別被膜のみ。撚線/撚撚線全体を包む円筒シースには以下が必要:
  - 外接円ベースのシース幾何（素線配置 → シース内径）
  - 最外層素線とシース内面の接触ペア生成
  - シース剛性寄与（円筒シェル近似 or 等価梁剛性）
  - シース面圧拘束（外圧として最外層素線に作用）
...
- **現在の CoatingModel は素線個別被膜のみ対応**。撚線/撚撚線全体を包むシース（外被）は未対応。シースは個別素線の梁要素に帰着できず、束全体への拘束（外圧）として作用するため、別の力学モデルが必要。
