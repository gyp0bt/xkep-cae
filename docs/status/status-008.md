...
4. **被膜モデルの範囲**: 目的を明確にする（剛性寄与 / 摩擦制御 / 保護）。
...
- [ ] 撚線モデルの最小状態変数の設計（用途決定が前提）
- [ ] Level 0: 軸方向素線＋penalty接触＋正則化Coulomb
- [ ] Level 1: θ_i 未知量化＋被膜平均化
- [ ] Level 2: 素線Cosserat rod化

### 確認事項

- [ ] 撚線モデルの第一ターゲット用途（曲げ？ねじり？疲労？）の決定→状態変数選定に直結
- [ ] 被膜モデルの目的（剛性寄与/摩擦制御/保護）の決定
- [ ] 接触ペア管理の具体的戦略（隣接のみ？代表接触？連続平均化？）
