...
3. **TODO 3（検証完了）**: frozen-m解消効果 — Hertz型で frac 0.40→0.62+（進行中）
...
| penalty_exponent | frac | incr | cutback |
|------------------|------|------|---------|
| 1.0（線形） | **0.40** | 208 | 14 |
| **1.5（Hertz型）** | **0.62+** | 300+ | 23+ |

### 分析

- 線形ペナルティ: status-295と同一（frac=0.40）、frozen-m解消（status-294/295）の恩恵なし
- **Hertz型ペナルティ: frac=0.62+**（300 incr時点、600 incr実行で0.79+に進行中）
- Hertz型ペナルティ + frozen-m解消の相乗効果で55%以上の改善
- frac=0.79付近でチャタリング発生（接触活性集合の微小変動）

### 参考: status-285との比較

status-285ではfrac=0.998を達成していたが、その後のfrozen-m解消（status-294/295）で
接線剛性の精度が向上した一方、NR挙動が変化。frac=0.62+は「正しい接線」での結果。
...
- [ ] frac=0.79+のstall原因分析（チャタリング vs 接線不整合）
