# status-314: プロファイル統計 API 強化 + BenchmarkRunner プロファイル自動キャプチャ
...
なお、本 status は当初 status-313 として作成されたが、master 側で並行して別の status-313（撚線ファイバー梁モデル設計仕様策定）がマージされたため、コンフリクト解消の際に status-314 へ付け替えた。内容は当初版と同一。
